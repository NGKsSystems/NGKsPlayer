...
class DemucsService {
  ...
  async separateStems(inputFile, outputDir, stemsCount = '4stems', progressCallback) {
    return new Promise((resolve, reject) => {
      ...
      this.activeProcess.on('close', (code) => {
        this.activeProcess = null;
        if (code !== 0 && code !== null) {
          reject(new Error(`Demucs process exited with code ${code}`));
        }
      });
  ...
  cancel() {
    if (this.activeProcess) {
      console.log('🛑 Cancelling Demucs process');
  ...
  async checkPythonInstallation() {
    try {
      ...
      const checkCmd = `${pythonPath} -c "import demucs; print('OK')"`;
      ...
      return {
        ...
        path: pythonPath,
        demucsInstalled: result.includes('OK')
      };
    } catch (error) {
      return {
        available: false,
...
module.exports = new DemucsService();
