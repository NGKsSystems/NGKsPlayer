...
function createWindow() {
  ...
  // createApplicationMenu(); // Commented out - menu bar is now hidden
...
register('library:renameFile', async (_evt, { oldPath, newName }) => {
  ...
  try {
    ...
    console.log('[library:renameFile] Raw input newName:', JSON.stringify(newName));
    console.log('[library:renameFile] originalExt value:', JSON.stringify(originalExt));
    console.log('[library:renameFile] originalExt length:', originalExt?.length);
    console.log('[library:renameFile] originalExt truthy?', !!originalExt);
    
    // CRITICAL: Preserve file extension if user didn't include it
    let finalNewName = newName.trim();
    const newNameHasExt = path.extname(finalNewName);
    console.log('[library:renameFile] newName after trim:', JSON.stringify(finalNewName));
    console.log('[library:renameFile] newNameHasExt:', JSON.stringify(newNameHasExt));
    console.log('[library:renameFile] Condition check: !newNameHasExt =', !newNameHasExt, ', originalExt =', !!originalExt);
    
    if (!newNameHasExt && originalExt) {
      finalNewName = finalNewName + originalExt;
      console.log('[library:renameFile] ✅ ADDED extension:', originalExt, '-> Final name:', finalNewName);
    } else if (newNameHasExt) {
      console.log('[library:renameFile] ❌ User provided extension, not adding:', newNameHasExt);
    } else {
      console.log('[library:renameFile] ❌ No original extension found to add');
    }
    
    const newPath = path.join(directory, finalNewName);
    console.log('[library:renameFile] Final paths:');
    console.log('  Original:', normalizedOldPath);
    console.log('  New:     ', newPath);
    console.log('  Extension logic: originalExt =', originalExt, ', newName =', newName, ', finalNewName =', finalNewName);
    ...
    try {
      ...
      console.log('[library:renameFile] File renamed successfully on disk');
    } catch (renameErr) {
      console.error('[library:renameFile] File rename failed:', renameErr);
      return { error: `Failed to rename file: ${renameErr.message}` };
    }

    // Only update database if file rename succeeded
