...
def detect_time_signature(audio_file):
    ...
    try:
        # Load first 90 seconds for better analysis
        y, sr = librosa.load(audio_file, sr=22050, mono=True, duration=90)
...
if __name__ == '__main__':
    ...
    # Only process files directly in the folder, not subdirectories
    for file in os.listdir(folder):
        filepath = os.path.join(folder, file)
        if os.path.isfile(filepath) and file.lower().endswith(extensions):
            count += 1
