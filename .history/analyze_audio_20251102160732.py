...
def detect_bpm_and_key(audio_file):
    ...
    try:
        ...
        bpm = float(tempo) if isinstance(tempo, (int, float, np.number)) else float(tempo[0]) if len(tempo) > 0 else 0
        
        # Key Detection using chromagram (use STFT instead of CQT for better compatibility)
        ...
        bpm_confidence = float(np.mean(pulse)) if len(pulse) > 0 else 0.5
