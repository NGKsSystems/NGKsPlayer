...
class RequestServer {
  constructor(db, mainWindow) {
    ...
    this.requestPolicy = 'free'; // 'free', 'paid', 'either'
  }
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'feedback':
          ...
          break;
        case 'tip':
          this.handleTip(data.amount, data.name, data.message, data.paymentMethod, data.paymentHandle);
          break;
      }
    } catch (error) {
  ...
  handleTip(amount, name, message, paymentMethod, paymentHandle) {
    const tipItem = {
      id: Date.now().toString(),
      amount,
      name: name || 'Anonymous',
  ...
  setPaymentHandles(handles) {
    ...
    console.log('[RequestServer] Payment handles updated:', this.paymentHandles);
  }
  ...
  setRequestPolicy(policy) {
    if (['free', 'paid', 'either'].includes(policy)) {
      this.requestPolicy = policy;
      console.log('[RequestServer] Request policy updated:', this.requestPolicy);
    }
  }
...
<body>
  <div class="container">
    ...
    ${this.requestPolicy === 'paid' ? `
    <div style="background: linear-gradient(145deg, #f39c12, #e67e22); padding: 15px; border-radius: 10px; margin-bottom: 20px; text-align: center; font-weight: 600;">
      💵 Paid Requests Only - Payment required (not guaranteed to be played)
    </div>
    ` : this.requestPolicy === 'either' ? `
    <div style="background: linear-gradient(145deg, #3498db, #2980b9); padding: 15px; border-radius: 10px; margin-bottom: 20px; text-align: center; font-weight: 600;">
      💰 Free or Paid Requests - Payment moves you up the queue (not guaranteed)
    </div>
    ` : ''}
    ...
    <div id="tipModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 1000; padding: 20px;">
      <div style="background: #1a1a2e; border-radius: 15px; padding: 20px; max-width: 500px; margin: 50px auto;">
        ...
        <div style="margin-top: 15px;">
          ...
          <div id="paymentButtons" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-bottom: 15px;">
            <!-- Payment buttons will be dynamically generated -->
          </div>
  ...
  <script>
    ...
    let paymentInfo = ${JSON.stringify(this.paymentHandles)};
    ...
    function showTipModal() {
      ...
      // Generate payment buttons dynamically based on configured methods
      const buttonsContainer = document.getElementById('paymentButtons');
      let buttonsHTML = '';
      
      const paymentMethods = [
        { key: 'venmo', name: 'Venmo', color: '#0088fe' },
        { key: 'cashapp', name: 'Cash App', color: '#00d96f' },
        { key: 'paypal', name: 'PayPal', color: '#003087' },
        { key: 'zelle', name: 'Zelle', color: '#652b92' }
      ];
      
      paymentMethods.forEach(method => {
        if (paymentInfo[method.key] && paymentInfo[method.key].trim() !== '') {
          buttonsHTML += \`
            <button onclick="selectPaymentMethod('\${method.key}')" class="payment-btn" 
              style="padding: 10px; background: rgba(\${parseInt(method.color.slice(1,3), 16)}, \${parseInt(method.color.slice(3,5), 16)}, \${parseInt(method.color.slice(5,7), 16)}, 0.2); 
              border: 2px solid \${method.color}; border-radius: 8px; color: white; font-weight: 600; cursor: pointer; 
      ...
      if (buttonsHTML === '') {
        buttonsHTML = '<p style="text-align: center; color: rgba(255,255,255,0.5); padding: 20px; grid-column: 1 / -1;">The DJ has not configured payment methods yet.</p>';
      }
    ...
    function selectPaymentMethod(method) {
      ...
      if (!paymentInfo[method] || paymentInfo[method].trim() === '') {
        alert('This payment method has not been configured by the DJ yet.');
        return;
      }
