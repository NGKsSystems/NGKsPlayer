...
class WhisperService {
  ...
  async transcribeAudio(filePath, modelSize = 'base', language = null, onProgress = null) {
    ...
    // Build command args - python script must come first, then the script's arguments
    ...
    return new Promise((resolve, reject) => {
      try {
        // DON'T use shell - it breaks argument parsing with spaces in filenames
        // spawn() will handle the arguments correctly without shell
        this.process = spawn(pythonPath, args, {
          shell: false,
          cwd: path.dirname(scriptPath),
          env: process.env,
          windowsVerbatimArguments: true  // Windows: don't quote/escape args
        });
