...
class WhisperService {
  ...
  findPython() {
    ...
    for (const pythonPath of possiblePaths) {
      try {
        const result = require('child_process').spawnSync(pythonPath, ['-c', 'import sys; print(sys.executable)'], { 
          encoding: 'utf8',
          shell: true 
        });
        
        if (result.status === 0 && result.stdout) {
          // Get the actual full path from sys.executable
          const fullPath = result.stdout.trim();
          console.log(`✅ Found Python: ${fullPath}`);
          return fullPath;
        }
      } catch (error) {
        // Try next path
  ...
  async transcribeAudio(filePath, modelSize = 'base', language = null, onProgress = null) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // Prepend ffmpeg path to PATH
        env.PATH = ffmpegPath + ';' + env.PATH;
        
        console.log('[Whisper] Added ffmpeg to PATH:', ffmpegPath);
        console.log('[Whisper] Using Python:', pythonPath);
        
        // Don't use shell - pass full Python path directly
        // This avoids "python is not recognized" errors
