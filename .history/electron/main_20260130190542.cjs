...
register('library:renameFile', async (_evt, { oldPath, newName }) => {
  ...
  try {
    ...
    if (!path.extname(finalNewName) && originalExt) {
      finalNewName = finalNewName + originalExt;
      console.log('[library:renameFile] Added extension:', originalExt, '-> Final name:', finalNewName);
    } else if (path.extname(finalNewName)) {
      console.log('[library:renameFile] User provided extension:', path.extname(finalNewName));
    } else {
      console.log('[library:renameFile] No original extension found, keeping name as-is');
    }
