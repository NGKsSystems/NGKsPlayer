...
logger.info('main.cjs', `App initialized: ${app.getName()} at ${app.getPath('userData')}`);
...
if (!appIsolation.enforceSingleInstance()) {
  ...
  logger.info('main.cjs', 'Exiting - another instance already running');
  process.exit(0);
}
...
try {
  ...
  logger.debug('main.cjs', 'RequestServer loaded successfully');
} catch (error) {
  logger.error('main.cjs', 'Failed to load RequestServer:', error);
  RequestServer = class DummyRequestServer {
    constructor() {
      logger.warn('DummyRequestServer', 'Using fallback - real RequestServer failed to load');
    }
...
async function loadAudioFile(filePath) {
  ...
  logger.verbose('REAL AUDIO', `Loading audio file: ${filePath}`);
