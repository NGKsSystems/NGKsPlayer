...
console.log('========================================================');
console.log('STARTING ELECTRON APP: NGKsPlayer');
console.log('Current Directory:', __dirname);
console.log('Process CWD:', process.cwd());
console.log('Package.json location:', path.join(__dirname, '..', 'package.json'));
console.log('========================================================');
...
console.log('[main.cjs] App name set to: NGKsPlayer');
console.log('[main.cjs] User data path: ', app.getPath('userData'));
...
console.log('[main.cjs] Initializing app isolation...');
if (!appIsolation.enforceSingleInstance()) {
  // Another instance is already running, exit
  console.log('[main.cjs] Exiting - another instance already running');
  process.exit(0);
}
appIsolation.initialize();
console.log(`[main.cjs] App ID: ${appIsolation.getAppId()}`);
...
try {
  ...
  console.log('[main.cjs] RequestServer loaded successfully');
} catch (error) {
  console.error('[main.cjs] Failed to load RequestServer:', error);
  RequestServer = class DummyRequestServer {
    constructor() {
      console.warn('[DummyRequestServer] Using fallback - real RequestServer failed to load');
    }
...
register('library:renameFile', async (_evt, { oldPath, newName }) => {
  try {
    console.log('[library:renameFile] Called with:', { oldPath, newName });
    ...
    console.log('[library:renameFile] Normalized old path:', normalizedOldPath);
    console.log('[library:renameFile] New path:', newPath);

    // Check if old file exists
    try {
      await fs.access(normalizedOldPath);
    } catch (err) {
      console.error('[library:renameFile] Old file does not exist:', normalizedOldPath);
    ...
    try {
      ...
      console.log('[library:renameFile] New file already exists');
    ...
    console.log(`[library:renameFile] Database updated. Changes: ${result.changes}`);
    console.log(`[library:renameFile] SUCCESS: Renamed ${normalizedOldPath} -> ${newPath}`);
...
// Rename a file and update database
try { ipcMain.removeHandler('library:renameFile'); } catch {}
ipcMain.handle('library:renameFile', async (_evt, { oldPath, newName }) => {
  try {
    console.log('[library:renameFile] Called with:', { oldPath, newName });
...
// UNIFIED ANALYZER - Single entry point for ALL audio analysis
// All analysis (single track, bulk, batch) routes through AudioAnalyzer in the renderer
register('library:analyzeTrack', async (_evt, { filePath, trackId }) => {
  console.log(`[Unified Analyzer] Analysis initiated for: ${filePath}`);
  return { success: true, trackId };
});
