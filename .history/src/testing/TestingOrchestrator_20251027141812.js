...
export class TestingOrchestrator {
  ...
  async runQuickTests() {
    ...
    try {
      ...
      await this.runTestCategory('essential', 'Essential system tests', ['--testNamePattern=essential|core|basic']);
  ...
  async runTestCategory(category, description, extraArgs = []) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const jestArgs = [
        ...
        'test',
        '--json',
        '--outputFile=' + path.join(this.options.outputDir, `${category}-results.json`),
        ...extraArgs
      ];
