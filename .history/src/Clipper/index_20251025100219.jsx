...
const Clipper = ({ onNavigate }) => {
  ...
  const handlePlayPause = () => {
    ...
    if (isPlayingRef.current) {
      ...
      // CRITICAL: Stop any existing playback COMPLETELY first
      if (updateIntervalRef.current) {
        clearInterval(updateIntervalRef.current);
        updateIntervalRef.current = null;
      }
      ...
      requestAnimationFrame(() => {
        ...
        const realStartTime = performance.now(); // Add this line

        source.start(0, playFromTime);
        sourceRef.current = source;

        // Track playback position - USE AUDIO SOURCE'S NATURAL TIMING
        let logCounter = 0;
        const interval = setInterval(() => {
          // Calculate time based on when we started and how much real time has passed
          
          // The most accurate way: use the Web Audio API's context time
          const audioElapsed = (audioContext.currentTime - startTime) * 1000;
          const realElapsed = performance.now() - realStartTime;
          const newTime = startingTimeMs + audioElapsed;
          
          // But let's compare with real time
          const realTimeNewTime = startingTimeMs + realElapsed;
          
          // Only log every 10 frames
          if (logCounter % 10 === 0) {
            console.log('Audio vs Real time:', { 
              audioElapsed,
              realElapsed,
              audioTime: newTime / 1000,
              realTime: realTimeNewTime / 1000,
              difference: (newTime - realTimeNewTime) / 1000
            });
          }
          logCounter++;

          // Use real time instead of audio context time
          if (realTimeNewTime >= (audioBuffer.duration * 1000)) {
          // Song ended
          if (updateIntervalRef.current) {
            clearInterval(updateIntervalRef.current);
            updateIntervalRef.current = null;
          ...
          console.log('SETTING currentTime to:', realTimeNewTime, 'seconds:', realTimeNewTime / 1000);
          setCurrentTime(realTimeNewTime); // Use real time, not audio context time
        }
          }
        }
