...
const ProClipperBrandNew = ({ onNavigate }) => {
  ...
  const generateWaveform = async (audioFile, audioDuration) => {
    try {
      ...
      const samples = Math.floor(audioDuration * 10); // 10 samples per second for width
    ...
    if (selectedFile) {
      ...
      if (audioRef.current) {
        ...
        audioRef.current.addEventListener('loadedmetadata', () => {
          ...
          generateWaveform(selectedFile, audioDuration); // Pass duration directly
        });
      }
    }
  };
  ...
  return (
    <div style={{ padding: '20px', background: '#000', color: '#fff', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '30px' }}>
        <h1 style={{ color: '#ff0000', fontSize: '32px' }}>🔥 PRO CLIPPER - FIXED & WORKING</h1>
