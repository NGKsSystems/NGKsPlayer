...
const WaveformEditor = ({
  ...
  useEffect(() => {
    ...
    // Clear entire canvas to remove old playhead
    ctx.fillStyle = '#0d0d0d';
    ctx.fillRect(0, 0, canvasWidth, canvasHeight);
    
    // Redraw the waveform (same as main effect but just the visible area)
    const data = audioBuffer.getChannelData(0);
    const totalSamples = data.length;
    const sampleRate = audioBuffer.sampleRate;
    const startSample = (visibleStart / 1000) * sampleRate;
  ...
  }, [currentTime, scrollOffset, viewDuration, duration, audioBuffer, inPoint, outPoint]);
