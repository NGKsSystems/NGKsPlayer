...
const WaveformEditor = ({
  ...
  const handleCanvasClick = (e) => {
    if (isDragging) return; // Don't scrub while dragging
  ...
  // Handle drag scrolling on canvas
  const handleCanvasMouseDown = (e) => {
    if (zoom <= 0.5) return; // No drag scrolling at full zoom
  ...
  }, [isDragging, dragStartX, dragStartOffset, zoom, viewDuration, duration]);

  // Handle mouse wheel scrolling on canvas
  const handleCanvasWheel = (e) => {
    if (zoom <= 0.5) return; // No scrolling at full zoom
  ...
  }, [zoom, scrollOffset, viewDuration, duration]);
