...
const WaveformEditor = ({
  ...
  useEffect(() => {
    ...
    const drawPlayhead = () => {
      ...
      // IMPORTANT: Save the entire canvas state before clearing
      // We'll restore it after, then draw playhead on top
      const imageData = ctx.getImageData(0, 0, canvasWidth, canvasHeight);

      // Clear just for the playhead area redraw
      // But restore the full content, then draw playhead on top
      ...
      if (currentTime >= visibleStart && currentTime <= visibleEnd) {
        ...
        // Draw clear area where playhead was
        ctx.clearRect(playX - 2, 0, 4, canvasHeight);
