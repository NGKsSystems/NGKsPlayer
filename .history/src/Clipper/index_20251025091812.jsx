...
const Clipper = ({ onNavigate }) => {
  ...
  const handlePlayPause = () => {
    ...
    if (isPlayingRef.current) {
      ...
      const interval = setInterval(() => {
        ...
        const newTime = currentTime + elapsedTime; // Fix: currentTime is already in ms
        console.log('Time calculation:', { currentTime, elapsedTime, newTime, displayTime: newTime / 1000 });
  ...
  const handleTimeChange = (newTime) => {
    ...
    if (isPlayingRef.current && sourceRef.current && audioBuffer) {
      ...
      const interval = setInterval(() => {
        const elapsedTime = (audioContext.currentTime - startTime) * 1000;
        const newTimeUpdate = playFromTime * 1000 + elapsedTime;
