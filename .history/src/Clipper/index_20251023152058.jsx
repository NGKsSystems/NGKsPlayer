...
const Clipper = ({ onNavigate }) => {
  ...
  const handlePlayPause = () => {
    ...
    if (isPlaying) {
      ...
      // CRITICAL: Prevent double-playing by checking if already playing
      if (sourceRef.current) {
        try {
          sourceRef.current.stop();
        } catch (e) {
      ...
      // Track playback position - FIXED to use actual audio context time
      const interval = setInterval(() => {
        const elapsedTime = (audioContext.currentTime - startTime) * 1000;
        const newTime = playFromTime * 1000 + elapsedTime;
