    ...
    if (isPlayingRef.current) {
      ...
      requestAnimationFrame(() => {
        ...
        const interval = setInterval(() => {
          ...
          const realElapsedMs = performance.now() - performance.now();
          
          // Instead of calculating, get the ACTUAL audio position
          try {
            ...
            // But let's slow it down to match real time
            const realTimeNewTime = startingTimeMs + (performance.now() - realStartTime);
            
            // Only log every 10 frames
            if (logCounter % 10 === 0) {
              console.log('Audio vs Real time:', { 
                audioElapsed,
                realElapsed: performance.now() - realStartTime,
          ...
          setCurrentTime(newTime); // RE-ENABLED with real-time tracking
        }
