...
const ProClipperV2 = ({ onNavigate }) => {
  ...
  const renderWaveform = () => {
    ...
    for (let i = 0; i < width; i += 3) { // More bars (every 3px instead of 4)
      // Use seed to make consistent waveform shape for same file
      const pseudoRandom = Math.sin(seed + i * 0.02) * 0.5 + 0.5;
      const height_val = pseudoRandom * height * 0.9 + 40; // Much bigger bars
      bars.push(
        <rect
          key={i}
          x={i}
