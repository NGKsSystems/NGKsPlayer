...
class AudioAnalyzer {
  ...
  intervalsToTempo(intervals, sampleRate) {
    ...
    const detectedBPM = this.findMostCommon(validBpms);
    
    // Check if this might be half-time - if most values cluster around half
    const doubledBpms = validBpms.map(bpm => bpm * 2).filter(bpm => bpm <= 180);
    if (doubledBpms.length > validBpms.length * 0.6) {
      const doubledMode = this.findMostCommon(doubledBpms);
      // If doubled version is more consistent, use it
      console.log('[AudioAnalyzer] Half-time detected, doubling BPM from', detectedBPM, 'to', doubledMode);
      return doubledMode;
    }
  ...
  matchKeyProfile(pitchClassProfile) {
    ...
    console.log('[AudioAnalyzer] Normalized profile:', normalized.map((v, i) => `${noteNames[i]}:${(v*100).toFixed(1)}%`).join(', '));
    ...
    console.log('[AudioAnalyzer] Major third (', noteNames[majorThirdIdx], '):', (majorThirdStrength*100).toFixed(1) + '%');
    console.log('[AudioAnalyzer] Minor third (', noteNames[minorThirdIdx], '):', (minorThirdStrength*100).toFixed(1) + '%');
    
    // Also check the fifth for validation
    const fifthIdx = (maxIdx + 7) % 12;
    console.log('[AudioAnalyzer] Fifth (', noteNames[fifthIdx], '):', (normalized[fifthIdx]*100).toFixed(1) + '%');
