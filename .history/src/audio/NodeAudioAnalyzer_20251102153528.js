/**
 * Node.js Adapter for AudioAnalyzer
 * 
 * Wraps browser-based AudioAnalyzer for use in Node.js calibration script
 * Uses 'web-audio-api' package to simulate browser AudioContext
 */
...
class NodeAudioAnalyzer {
  ...
  /**
   * Analyze audio file - Node.js compatible version
   */
  async analyzeAudioFile(filePath) {
    try {
      ...
      // Decode audio
      const audioBuffer = await this.audioContext.decodeAudioData(fileBuffer.buffer);
  ...
  async detectBPM(audioBuffer) {
    try {
      ...
      // Step 3: Find most common interval (tempo)
      ...
      // Step 5: Validate BPM range (60-220 BPM)
      if (bpm < 60 || bpm > 220) {
        // Try octave correction
        if (bpm < 60) return { bpm: bpm * 2, confidence: 0.7 };
        if (bpm > 220) return { bpm: bpm / 2, confidence: 0.7 };
      }
