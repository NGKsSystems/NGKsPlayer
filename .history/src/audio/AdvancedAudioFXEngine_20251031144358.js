...
class AdvancedAudioFXEngine {
  ...
  initializeFXChains() {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      // Connect chain to engine's master input/output
      // Engine input -> chain input (audio flows in)
      this.input.connect(chain.input);
      // Chain output -> engine output (audio flows out)
      chain.output.connect(this.output);
      
      this.fxChains.set(i, chain);
      console.log(`🔗 FX Chain ${i} connected to engine`);
    }
