...
class AudioAnalyzer {
  ...
  async detectKey(audioBuffer) {
    ...
    try {
      // Use Web Audio API's AnalyserNode for efficient FFT
      ...
      analyser.fftSize = 8192; // High resolution for accurate pitch detection
      analyser.smoothingTimeConstant = 0;
      
      source.connect(analyser);
      source.start(0);

      // Analyze in chunks (every 0.5 seconds for 30 seconds max)
      const sampleRate = audioBuffer.sampleRate;
      const duration = Math.min(audioBuffer.duration, 30);
      const numSamples = Math.floor(duration * 2); // 2 samples per second
      const pitchClassProfile = new Array(12).fill(0);

      for (let i = 0; i < numSamples; i++) {
        const time = i * 0.5;
        const sampleOffset = Math.floor(time * sampleRate);
        
        if (sampleOffset >= audioBuffer.length) break;
        ...
        this.mapFrequenciesToPitchClasses(frequencyData, sampleRate, analyser.frequencyBinCount, pitchClassProfile);
      }
      ...
      console.log('[AudioAnalyzer] Pitch class profile:', pitchClassProfile.map(v => v.toFixed(2)));
      return key;
    } catch (error) {
  ...
  mapFrequenciesToPitchClasses(frequencyData, sampleRate, binCount, pitchClassProfile) {
    ...
    for (let bin = 0; bin < binCount; bin++) {
      ...
      // Focus on musical range (80 Hz to 2000 Hz)
      if (frequency < 80 || frequency > 2000) continue;
      
      const energy = frequencyData[bin];
      if (energy < 30) continue; // Ignore quiet bins
      ...
      // Weight by energy
      pitchClassProfile[normalizedPitchClass] += energy;
    }
  }
