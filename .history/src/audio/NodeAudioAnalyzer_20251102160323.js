 ...
 * Industry-standard audio analysis
 */
...
export default NodeAudioAnalyzer;
  ...
  async detectBPM(audioData) {
    try {
      ...
      // Minimum confidence threshold - return null if too uncertain
      if (bestCandidate.confidence < 0.15) {
        return { bpm: null, confidence: 0 };
      }
  ...
  /**
   * Calculate energy envelope with spectral flux for better beat detection
   */
  calculateEnergyEnvelope(samples, sampleRate) {
    ...
    for (let i = 0; i < samples.length - windowSize; i += hopSize) {
      ...
      // Calculate spectral flux (change in energy) for better onset detection
      const flux = Math.max(0, energy - prevEnergy);
      
      // Combine energy and flux for better beat sensitivity
      const combined = energy * 0.6 + flux * 0.4;
      
      envelope.push(combined);
      prevEnergy = energy;
    }
  ...
  /**
   * Compute chromagram using FFT with proper frequency weighting
   */
  computeChromagram(samples, sampleRate) {
    ...
    if (numFrames < 1) return chromagram;
    
    for (let frame = 0; frame < numFrames; frame++) {
      const start = frame * hopSize;
      const segment = new Array(fftSize);
      
      // Apply Hanning window
      for (let i = 0; i < fftSize; i++) {
        const sampleVal = start + i < samples.length ? samples[start + i] : 0;
        segment[i] = sampleVal * 0.5 * (1 - Math.cos(2 * Math.PI * i / (fftSize - 1)));
      }
      
      // Compute magnitude spectrum
      const spectrum = this.computeFFT(segment);
      
      // Map spectrum to chroma bins (pitch classes) with logarithmic weighting
      for (let bin = 1; bin < spectrum.length / 2; bin++) {
        const frequency = (bin * sampleRate) / fftSize;
        
        // Focus on harmonic content (80Hz to 2000Hz - musical fundamentals and harmonics)
        if (frequency < 80 || frequency > 2000) continue;
        
        const magnitude = spectrum[bin];
        if (magnitude < 0.001) continue; // Skip very quiet bins
        
        const chromaBin = this.frequencyToChroma(frequency);
        
        // Weight by magnitude with logarithmic scaling to emphasize stronger tones
        const weight = Math.log(1 + magnitude * 1000);
        chromagram[chromaBin] += weight;
      }
    }
