...
class AudioAnalyzer {
  ...
  intervalsToTempo(intervals, sampleRate) {
    ...
    // Auto-correct if outside typical range (70-160 BPM)
    // Most music falls in this range except extreme cases
    if (detectedBPM < 70) {
      console.log('[AudioAnalyzer] BPM too low (', detectedBPM, '), doubling to', detectedBPM * 2);
      detectedBPM = detectedBPM * 2;
    } else if (detectedBPM > 160) {
      console.log('[AudioAnalyzer] BPM too high (', detectedBPM, '), halving to', detectedBPM / 2);
      detectedBPM = detectedBPM / 2;
    }
