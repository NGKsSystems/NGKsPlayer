...
class AudioAnalyzer {
  ...
  matchKeyProfile(pitchClassProfile) {
    ...
    // Weighted scale profiles: tonic=3, fifth=2, third=2, others=1, chromatic=-1
    // Major scale: Root, 2, 3, 4, 5, 6, 7
    const majorProfile = [3, -1, 1, -1, 2, 1, -1, 2, -1, 1, -1, 1]; // C major
    // Minor scale: Root, 2, b3, 4, 5, b6, b7
    const minorProfile = [3, -1, 1, 2, -1, 1, -1, 2, 1, -1, 1, -1]; // C minor
    ...
    for (let keyIdx = 0; keyIdx < 12; keyIdx++) {
      // Test major key - weighted sum
      const rotatedMajor = this.rotateArray(majorProfile, keyIdx);
      let majorScore = 0;
      for (let i = 0; i < 12; i++) {
        majorScore += normalized[i] * rotatedMajor[i];
      }
