...
class AudioAnalyzer {
  ...
  async detectKey(audioBuffer) {
    ...
    try {
      ...
      // Match to key using scale templates
      let key = this.matchKeyProfile(chromaProfile);
      
      // Apply tuning correction if significantly detuned
      if (Math.abs(tuningOffset) > 0.3) {
        const semitoneShift = Math.round(tuningOffset);
        const correctedKey = this.transposeKey(key, -semitoneShift);
        console.log('[AudioAnalyzer] Tuning-corrected key:', key, '→', correctedKey, '(', -semitoneShift, 'semitones)');
        key = correctedKey;
      }
  ...
  matchKeyProfile(pitchClassProfile) {
    ...
    const minorScale = [3, 0, 1, 2, 0, 1, 0, 2, 2, 0, 1, 0]; // C minor - weighted (tonic=3, minor third=2, fifth=2)
    ...
    for (let i = 0; i < 5; i++) {
      console.log(`  ${i+1}. ${scores[i].key}: ${scores[i].score.toFixed(4)}`);
    }
  ...
  estimateTuning(channelData, sampleRate) {
    ...
    // Calculate average tuning offset
    const tuningOffsets = peakFrequencies.map(freq => {
      // Find nearest A (220, 440, 880)
      const nearestA = freq < 330 ? 220 : (freq < 660 ? 440 : 880);
      return 12 * Math.log2(freq / nearestA);
    });
