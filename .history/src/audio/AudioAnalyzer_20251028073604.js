...
class AudioAnalyzer {
  ...
  addChromaFromFrame(frameData, sampleRate, chromaProfile) {
    // Frequency range: 65.4 Hz (C2) to 2093 Hz (C7) - 5 octaves
    const minFreq = 65.4;
    const maxFreq = 2093;
    const numHarmonics = 3; // Consider fundamental + 2 harmonics
    
    // For each pitch class, analyze multiple octaves with harmonic weighting
    for (let pitchClass = 0; pitchClass < 12; pitchClass++) {
      let totalMagnitude = 0;
      let totalWeight = 0;
      
      // Analyze across octaves (C2 to C7)
      for (let octave = 2; octave <= 7; octave++) {
        const fundamentalFreq = this.getPitchFrequency(pitchClass, octave);
        
        if (fundamentalFreq < minFreq || fundamentalFreq > maxFreq) continue;
        
        // Sum fundamental + harmonics with decreasing weight
        for (let harmonic = 1; harmonic <= numHarmonics; harmonic++) {
          const freq = fundamentalFreq * harmonic;
          
          if (freq > maxFreq) break;
          
          const magnitude = this.getMagnitudeAtFrequency(frameData, sampleRate, freq);
          
          // Harmonic weighting: fundamental = 1.0, 2nd harmonic = 0.5, 3rd = 0.33
          const harmonicWeight = 1.0 / harmonic;
          
          // Octave weighting: favor mid-range (octaves 3-5)
          const octaveWeight = octave >= 3 && octave <= 5 ? 1.5 : 1.0;
          
          const weight = harmonicWeight * octaveWeight;
          
          totalMagnitude += magnitude * weight;
          totalWeight += weight;
        }
      }
      
      // Store weighted average
      chromaProfile[pitchClass] += totalWeight > 0 ? totalMagnitude / totalWeight : 0;
    }
  }
