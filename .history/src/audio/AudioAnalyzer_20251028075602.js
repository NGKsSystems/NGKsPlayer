...
class AudioAnalyzer {
  ...
  matchKeyProfile(pitchClassProfile) {
    ...
    for (let keyIdx = 0; keyIdx < 12; keyIdx++) {
      ...
      const majorCorr = this.correlate(normalized, rotatedMajor);
      correlations.push({ key: keys[keyIdx], corr: majorCorr });
      ...
      const minorCorr = this.correlate(normalized, rotatedMinor);
      correlations.push({ key: keys[keyIdx] + 'm', corr: minorCorr });
    ...
    for (let i = 0; i < 5; i++) {
      console.log(`  ${i+1}. ${correlations[i].key}: ${correlations[i].corr.toFixed(4)}`);
    }
