...
class AudioAnalyzer {
  ...
  intervalsToTempo(intervals, sampleRate) {
    ...
    if (detectedBPM < 70) {
      ...
      // If doubled version is more consistent (lower variance), use it
      if (doubledStdDev < stdDev * 1.2) { // 20% tolerance
        console.log('[AudioAnalyzer] Doubled BPM more consistent (', doubledStdDev.toFixed(2), '<', stdDev.toFixed(2), '), using', doubled);
        detectedBPM = doubled;
      } else {
      ...
      // If halved version is more consistent, use it
      if (halvedStdDev < stdDev * 1.2) { // 20% tolerance
        console.log('[AudioAnalyzer] Halved BPM more consistent (', halvedStdDev.toFixed(2), '<', stdDev.toFixed(2), '), using', halved);
        detectedBPM = halved;
      } else {
