...
class BWFCodec {
  ...
  static generateBWF(audioBuffer, metadata = {}, options = {}) {
    ...
    // Simple BWF structure calculation
    // RIFF header (12) + bext chunk header (8) + bext data (602) + fmt chunk (24) + data chunk header (8) + data
    const bextDataSize = 602;
    const riffHeaderSize = 12; // 'RIFF' + size + 'WAVE'
    const bextChunkSize = 8 + bextDataSize; // chunk header + data
    const fmtChunkSize = 24; // fmt chunk header (8) + fmt data (16)
    const dataChunkHeaderSize = 8; // 'data' + size
    
    const totalSize = riffHeaderSize + bextChunkSize + fmtChunkSize + dataChunkHeaderSize + dataLength;
