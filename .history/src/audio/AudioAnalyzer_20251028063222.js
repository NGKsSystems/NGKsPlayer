...
class AudioAnalyzer {
  ...
  async detectKey(audioBuffer) {
    ...
    try {
      // Use a simplified approach - analyze only first 30 seconds
      const maxSamples = audioBuffer.sampleRate * 30;
      const channelData = this.getMonoData(audioBuffer);
      const analyzeData = channelData.slice(0, Math.min(maxSamples, channelData.length));
      
      // Simplified key detection using frequency analysis
      const key = this.detectKeySimplified(analyzeData, audioBuffer.sampleRate);
  ...
  // Simplified key detection (faster, less accurate)
  detectKeySimplified(channelData, sampleRate) {
    const pitchClasses = new Array(12).fill(0);
    const windowSize = 4096;
    const hopSize = 2048;
    
    // Analyze fewer frames for speed
    const numFrames = Math.min(100, Math.floor((channelData.length - windowSize) / hopSize));
    
    for (let frame = 0; frame < numFrames; frame++) {
      const startIdx = frame * hopSize;
      const frameData = channelData.slice(startIdx, startIdx + windowSize);
      
      // Simple peak detection in frequency domain
      for (let i = 1; i < frameData.length - 1; i++) {
        if (Math.abs(frameData[i]) > 0.1) {
          // Estimate pitch class from position
          const pitchClass = i % 12;
          pitchClasses[pitchClass] += Math.abs(frameData[i]);
        }
      }
    }
    ...
    const isMinor = pitchClasses[(dominantPitch + 3) % 12] > pitchClasses[(dominantPitch + 4) % 12];
    
    return keys[dominantPitch] + (isMinor ? 'm' : '');
  }
