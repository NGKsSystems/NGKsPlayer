...
export class ModernLightingController {
  ...
  async sendUniverse(universeData) {
    ...
    try {
      const protocolInterface = this.protocols[this.activeProtocol];
      await protocolInterface.sendUniverse(universeData);
  ...
  async switchToFallbackProtocol() {
    ...
    if (currentIndex < 2) {
      ...
      try {
        const protocolInterface = this.protocols[nextProtocol];
        const success = await protocolInterface.initialize();
