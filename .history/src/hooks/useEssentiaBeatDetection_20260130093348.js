...
import EssentiaClass from 'essentia.js/dist/essentia.js-core.es.js';  // Core class
import EssentiaWASMFactory from 'essentia.js/dist/essentia-wasm.web.js';  // WASM factory

let essentiaInstance = null;

/**
 * Load Essentia.js WASM using correct pattern for v0.1.3
 */
async function loadEssentia() {
  if (essentiaInstance) return essentiaInstance;

  try {
    // 1. Load the WASM factory (this is the .web.js browser loader)
    const wasmFactory = EssentiaWASMFactory;  // No () here — it's already the factory

    // 2. Call the factory to get the initialized WASM module
    const wasmModule = await wasmFactory();   // This returns the Emscripten module

    // 3. Instantiate Essentia with the loaded module
