  ...
  useEffect(() => {
    ...
    if (!essentiaRef.current) {
      const loadEssentia = async () => {
        try {
          // Use UMD builds which work better with Vite/Electron
          const wasmModule = await import('essentia.js/dist/essentia-wasm.umd.js');
          const coreModule = await import('essentia.js/dist/essentia.js-core.umd.js');
          
          if (!mounted) return;
          
          // UMD modules export via .default in ES module context
          const EssentiaWASM = wasmModule.default || wasmModule.EssentiaWASM;
          const Essentia = coreModule.default || coreModule.Essentia;
          
          console.log('[Essentia] WASM module type:', typeof EssentiaWASM);
          console.log('[Essentia] WASM has EssentiaJS:', typeof EssentiaWASM?.EssentiaJS);
          
          // Create Essentia instance with WASM module
