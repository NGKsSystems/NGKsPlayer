  ...
  useEffect(() => {
    ...
    if (!essentiaRef.current) {
      const loadEssentia = async () => {
        try {
          // Try loading Essentia - version 0.1.3 has a different API
          const EssentiaModule = await import('essentia.js');
          
          if (!mounted) return;
          
          // For essentia.js 0.1.3, the module structure is different
          // It might export { Essentia } or be the default export
          let essentiaInstance;
          
          if (EssentiaModule.Essentia) {
            // Newer API
            essentiaInstance = new EssentiaModule.Essentia();
          } else if (EssentiaModule.default) {
            // Default export
            essentiaInstance = EssentiaModule.default;
          } else {
            // Try direct module
            essentiaInstance = EssentiaModule;
          }
          
          essentiaRef.current = essentiaInstance;
          setIsReady(true);
          console.log('✅ Essentia.js loaded and ready', essentiaInstance);
        } catch (err) {
          console.error('❌ Essentia load failed:', err);
          console.warn('🔄 Beat pulsing will use legacy bass detection only');
          // Don't set isReady - this will prevent Essentia features from running
        }
      };
