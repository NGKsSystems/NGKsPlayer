...
export const useWidgetPosition = (widgetId, initialPosition = { x: 0, y: 0, width: 200, height: 150 }) => {
  ...
  const [position, setPosition] = useState(() => {
    ...
    return savedWidgets[widgetId] || initialPosition;
  });
  ...
  useEffect(() => {
    const savedWidgets = getCurrentLayoutWidgets();
    const savedPosition = savedWidgets[widgetId];
    
    if (!savedPosition) return;
    
    const currentStr = JSON.stringify(position);
    const savedStr = JSON.stringify(savedPosition);
    
    if (currentStr !== savedStr) {
      setPosition(savedPosition);
    }
  }, [widgetId]); // Only run on mount and widgetId change
