...
/**
 * Load Essentia.js WASM - TEMPORARILY DISABLED
 */
async function loadEssentia() {
  if (essentiaInstance) return essentiaInstance;

  try {
    console.warn('[Essentia] WASM loading temporarily disabled to avoid download dialog');
    ...
    /* ORIGINAL CODE - COMMENTED OUT UNTIL WASM LOADING IS FIXED
    // Dynamically import the modules to avoid download prompts
    const [EssentiaModule, EssentiaWASMModule] = await Promise.all([
      import('essentia.js/dist/essentia.js-core.es.js'),
      import('essentia.js/dist/essentia-wasm.web.js')
    ]);

    // Get the Essentia class and WASM factory
    const EssentiaClass = EssentiaModule.default || EssentiaModule.Essentia;
    const EssentiaWASMFactory = EssentiaWASMModule.default || EssentiaWASMModule;

    console.log('[Essentia] Loading WASM module...');
    
    // Call the factory to get the initialized WASM module
    const wasmModule = await EssentiaWASMFactory();

    // Instantiate Essentia with the loaded module
