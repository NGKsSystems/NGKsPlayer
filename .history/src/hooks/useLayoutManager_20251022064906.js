...
export const useLayoutManager = () => {
  ...
  const registerWidget = useCallback((id, box) => {
    setLayouts(prev => {
      const next = { ...prev };
      const lay = next[currentLayout] || { timestamp: Date.now(), widgets: {} };
      lay.widgets = { ...lay.widgets, [id]: { ...(lay.widgets[id] || {}), ...box } };
      next[currentLayout] = lay;
      localStorage.setItem('dj-widget-layouts', JSON.stringify(next));
      return next;
    });
  }, [currentLayout]);
  ...
  const updateWidget = useCallback((id, patch) => {
    setLayouts(prev => {
      const next = { ...prev };
      const lay = next[currentLayout] || { timestamp: Date.now(), widgets: {} };
      const prevBox = lay.widgets[id] || {};
      lay.widgets = { ...lay.widgets, [id]: { ...prevBox, ...patch } };
      next[currentLayout] = lay;
      localStorage.setItem('dj-widget-layouts', JSON.stringify(next));
      return next;
    });
