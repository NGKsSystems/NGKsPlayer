  ...
  useEffect(() => {
    ...
    if (!essentiaRef.current) {
      const loadEssentia = async () => {
        try {
          // Dynamic import - essentia.js exports { Essentia, EssentiaWASM }
          const { Essentia, EssentiaWASM } = await import('essentia.js');
          
          if (!mounted) return;
          
          // Correct initialization for essentia.js 0.1.3:
          // Create Essentia instance passing the WASM module
          const essentia = new Essentia(EssentiaWASM);
          
          // Wait for WASM to be ready
          await essentia.module.ready;
          ...
          console.warn('🔄 This should not happen - check essentia.js installation');
          setIsReady(false);
        }
      };
