  ...
  useEffect(() => {
    ...
    if (!essentiaRef.current) {
      const loadEssentia = async () => {
        try {
          // Load the ES modules directly instead of CommonJS index
          ...
          // The ES module default export is what we need
          const wasmModule = EssentiaWASM.default;
          const EssentiaClass = EssentiaCore.default;
          
          // Create Essentia instance with the WASM module
          const essentia = new EssentiaClass(wasmModule);
          
          essentiaRef.current = essentia;
          setIsReady(true);
          console.log('✅ Essentia.js WASM loaded and ready', essentia);
        } catch (err) {
