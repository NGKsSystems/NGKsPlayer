  ...
  useEffect(() => {
    ...
    if (!essentiaRef.current) {
      const loadEssentia = async () => {
        try {
          // Import WASM (.web.js for browser) and Core (.es.js)
          ...
          // EssentiaWASM in .web.js is a factory function that returns a Promise
          const wasmInstance = await EssentiaWASM();
          
          if (!mounted) return;
          
          console.log('[Essentia] WASM initialized, has EssentiaJS:', typeof wasmInstance?.EssentiaJS);
