...
export const useLayoutManager = () => {
  ...
  const registerWidget = useCallback((id, box) => {
    ...
    setLayouts(prev => {
      ...
      if (current && 
          ...
          current.minimized === box.minimized) {
      ...
      setTimeout(() => saveToStorage(next), 1000);
      return next;
    });
  }, [currentLayout, saveToStorage]);
  ...
  const updateWidget = useCallback((id, patch) => {
    ...
    setLayouts(prev => {
      const current = prev[currentLayout]?.widgets?.[id];
      if (!current) return prev; // Widget not registered
      
      // Check if update is actually needed
      const needsUpdate = Object.entries(patch).some(
