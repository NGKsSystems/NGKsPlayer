  ...
  useEffect(() => {
    ...
    if (!essentiaRef.current) {
      const loadEssentia = async () => {
        try {
          // Import WASM (UMD) and Core (ES module)
          ...
          // Extract the actual exports (handle both UMD and ES6 default/named exports)
          const EssentiaWASMFactory = wasmModule.EssentiaWASM || wasmModule.default;
          const Essentia = coreModule.Essentia || coreModule.default;
          
          console.log('[Essentia] EssentiaWASM factory type:', typeof EssentiaWASMFactory);
          
          // Emscripten modules need to be initialized - they return a Promise
          // The module is ready when the Promise resolves
          let initializedWASM;
          if (typeof EssentiaWASMFactory === 'function') {
            // Factory function pattern
            initializedWASM = await EssentiaWASMFactory();
          } else if (EssentiaWASMFactory && typeof EssentiaWASMFactory.then === 'function') {
            // Already a Promise
            initializedWASM = await EssentiaWASMFactory;
          } else if (EssentiaWASMFactory && EssentiaWASMFactory.ready) {
            // Has a ready promise
            initializedWASM = await EssentiaWASMFactory.ready;
          } else {
            // Use as-is
            initializedWASM = EssentiaWASMFactory;
          }
          
          if (!mounted) return;
          
          console.log('[Essentia] WASM initialized, has EssentiaJS:', typeof initializedWASM?.EssentiaJS);
          
          // Create Essentia instance with initialized WASM
          const essentia = new Essentia(initializedWASM);
