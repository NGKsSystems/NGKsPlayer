...
export const useLayoutManager = () => {
  ...
  useEffect(() => {
    ...
    const layoutString = JSON.stringify(state.layouts);
    const currentLayoutString = state.currentLayout;
    ...
    storageDebounceRef.current = setTimeout(() => {
      try {
        localStorage.setItem('dj-widget-layouts', layoutString);
        localStorage.setItem('dj-current-layout', currentLayoutString);
      } catch (error) {
  ...
  const widgetsRef = useRef(state.layouts[state.currentLayout]?.widgets || {});
  
  useEffect(() => {
    widgetsRef.current = state.layouts[state.currentLayout]?.widgets || {};
  }, [state.layouts, state.currentLayout]);

  const getCurrentLayoutWidgets = useCallback(() => widgetsRef.current, []);
  const autoLoadLayout = useCallback(() => widgetsRef.current || null, []);
...
export const useWidgetPosition = (widgetId, initialPosition = { x: 0, y: 0, width: 200, height: 150 }) => {
  const { getCurrentLayoutWidgets, updateWidget } = useLayoutManager();
  const [position, setPosition] = useState(initialPosition);
  const isInitialized = useRef(false);
  const prevWidgets = useRef(null);
  ...
  useEffect(() => {
    ...
    const savedWidgetStr = JSON.stringify(savedWidgets[widgetId]);
    const prevWidgetStr = prevWidgets.current;
    
    if (savedWidgets[widgetId] && savedWidgetStr !== prevWidgetStr) {
      setPosition(savedWidgets[widgetId]);
      prevWidgets.current = savedWidgetStr;
    }
  ...
  const updatePosition = useCallback((newPosition) => {
    ...
    // Batch the updateWidget call to prevent extra re-renders
    queueMicrotask(() => {
      updateWidget(widgetId, updatedPosition);
    });
  }, [position, updateWidget, widgetId]);
