  ...
  useEffect(() => {
    ...
    if (!essentiaRef.current) {
      const loadEssentia = async () => {
        try {
          // Dynamic import to handle async loading
          const EssentiaModule = await import('essentia.js');
          
          if (!mounted) return;
          
          // The module exports EssentiaWASM which needs to be initialized
          const EssentiaWASM = EssentiaModule.EssentiaWASM || EssentiaModule.default;
          
          // Initialize the WASM module first
          const essentia = new EssentiaWASM.Essentia(EssentiaWASM);
          
          essentiaRef.current = essentia;
          setIsReady(true);
          console.log('✅ Essentia.js loaded and ready');
        } catch (err) {
          console.error('❌ Essentia load failed:', err);
          console.warn('🔄 Falling back to legacy beat detection');
          // Don't throw - just skip Essentia features
          setIsReady(false);
        }
      };
