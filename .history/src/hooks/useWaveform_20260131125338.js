  ...
  useEffect(() => {
    ...
    const drawWaveform = () => {
      ...
      const frequencyData = new Uint8Array(bufferLength);
            ...
            // Apply Chromatic Chaos effects if this theme is active
      if (window.currentThemeEffect) {
        try {
          window.currentThemeEffect(
            waveformCanvasRef.current,     // your canvas
            ctx,                           // your 2D context
            beatDetectionRef.current?.beatStrength || 0,  // strength
            peakRotation || false          // super peak flag
          );
        } catch (err) {
