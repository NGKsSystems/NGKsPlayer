...
class ExtremeVisualEffects {
  ...
  renderAurora(p) {
    ...
    // Aurora wave effect
    const waveX = x + Math.sin(Date.now() * 0.001 + waveOffset) * 30;
    const waveY = y + Math.cos(Date.now() * 0.0008 + waveOffset) * 20;
    
    // Glowing aurora band
    this.ctx.shadowColor = `${color}0.8)`;
    this.ctx.shadowBlur = size * 4;
    
    const gradient = this.ctx.createLinearGradient(waveX - size, waveY, waveX + size, waveY);
    gradient.addColorStop(0, `${color}0)`);
    gradient.addColorStop(0.5, `${color}${alpha})`);
    gradient.addColorStop(1, `${color}0)`);
    
    this.ctx.fillStyle = gradient;
    this.ctx.fillRect(waveX - size, waveY - size/4, size * 2, size/2);
