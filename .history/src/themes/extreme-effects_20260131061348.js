...
class ExtremeVisualEffects {
  ...
  activate() {
    ...
    // Add aurora background for hyperAurora theme
    if (this.currentTheme === 'hyperAurora') {
  ...
  cleanupAuroraEffects() {
    ...
    if (auroraBackground) {
      auroraBackground.remove();
    }
    
    // Remove aurora text effects
    const auroraElements = document.querySelectorAll('.aurora-title');
    auroraElements.forEach(el => {
      el.classList.remove('aurora-title', 'pulse-enhanced');
    });
    
    // Reset CSS variables
    document.documentElement.style.removeProperty('--aurora-intensity');
  }

  cleanupAllEffects() {
    // Clean up aurora effects
    this.cleanupAuroraEffects();
    
    // Remove RGB split filter
    document.body.style.filter = 'none';
    
    // Clean up all special effect elements
    const specialElements = document.querySelectorAll(
      '.blood-drip, .aurora-flare, .diamond-explosion, .nightmare-face, ' +
      '.lightning-bolt, .chromatic-split, .quantum-ripple'
    );
    specialElements.forEach(el => el.remove());
    
    // Remove theme-specific CSS classes
    document.body.classList.remove(
      'cyber-bloodbath', 'hyper-aurora', 'diamond-rain', 
      'neon-nightmare', 'galactic-storm', 'chromatic-chaos', 'quantum-drift'
    );
    ...
    if (this.specialEffectInterval) {
      clearInterval(this.specialEffectInterval);
      this.specialEffectInterval = null;
    }
  ...
  startNorthernLights() {
    // Create aurora background and text effects
    this.createAuroraBackground();
    this.setupAuroraTextEffects();
  ...
  spawnAuroraWave() {
    // Create fewer, wider vertical curtains spanning most of screen height
    const curtainCount = 2 + Math.floor(Math.random() * 3);
    ...
    for (let i = 0; i < curtainCount; i++) {
      const curtain = {
        ...
        vx: 0,
        vy: 0,
        width: 80 + Math.random() * 150, // Much wider curtains
        height: this.canvas.height * (0.8 + Math.random() * 0.2), // Most of screen height
        life: 1.0,
        decay: 0.002, // Slower decay
        color: this.getAuroraColor(),
        wavePhase: Math.random() * Math.PI * 2,
        intensity: 0.4 + Math.random() * 0.3, // Lower intensity
        type: 'aurora',
        curtainIndex: i,
        flowSpeed: 0.02 + Math.random() * 0.02
      };
      this.particles.push(curtain);
    }
  }
  ...
  createAuroraBackground() {
    // Create moving aurora background
    const auroraOverlay = document.createElement('div');
    auroraOverlay.className = 'aurora-bg';
    auroraOverlay.id = 'aurora-background';
    document.body.insertBefore(auroraOverlay, document.body.firstChild);
    
    // Set CSS variables for pulse control
    document.documentElement.style.setProperty('--aurora-intensity', '0.4');
  }
  
  setupAuroraTextEffects() {
    // Apply rainbow effects to track titles and up next
    const selectors = ['.np-track-title', '.up-next-title', '.track-info h3'];
    selectors.forEach(selector => {
      const elements = document.querySelectorAll(selector);
      elements.forEach(el => {
        el.classList.add('aurora-title');
      });
    });
  }
  ...
  triggerAuroraFlare() {
    ...
    flare.className = 'aurora-flare';
    document.body.appendChild(flare);
    
    // Intensify background aurora
    document.documentElement.style.setProperty('--aurora-intensity', '0.8');
    
    // Speed up title rainbow effect
    const titles = document.querySelectorAll('.aurora-title');
    titles.forEach(title => title.classList.add('pulse-enhanced'));
    
    setTimeout(() => {
      document.documentElement.style.setProperty('--aurora-intensity', '0.4');
      titles.forEach(title => title.classList.remove('pulse-enhanced'));
      flare.remove();
    }, 600);
  }
  
  spawnAuroraParticleTrails() {
    // Spawn rising aurora particles from waveform baseline
    const waveformRect = document.querySelector('.waveform-container')?.getBoundingClientRect();
    if (!waveformRect) return;
    
    const baselineY = waveformRect.top + waveformRect.height / 2;
    
    for (let i = 0; i < 8; i++) {
      const x = waveformRect.left + Math.random() * waveformRect.width;
      
      const particle = {
        x: x - this.canvas.getBoundingClientRect().left,
        y: baselineY - this.canvas.getBoundingClientRect().top,
        vx: (Math.random() - 0.5) * 2, // Slight horizontal drift
        vy: -2 - Math.random() * 3, // Upward movement
        size: 3 + Math.random() * 4,
        life: 1.0,
        decay: 0.008,
        color: Math.random() > 0.5 ? 'rgba(0, 255, 159, ' : 'rgba(255, 0, 255, ',
        type: 'auroraTrail',
  ...
  getChromaticColor() {
    const colors = [
      'rgba(255, 255, 255, ',  // Chrome white
      'rgba(192, 192, 192, ',  // Chrome silver
      'rgba(128, 128, 128, ',  // Chrome gray
      'rgba(0, 255, 255, ',    // Cyan chrome
      'rgba(255, 0, 255, ',    // Magenta chrome
      'rgba(255, 255, 0, ',    // Yellow chrome
      'rgba(224, 224, 224, '   // Light chrome
    ];
    return colors[Math.floor(Math.random() * colors.length)];
  }
  ...
  updateParticles() {
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      if (p.type === 'arterial') {
        ...
        p.vy *= 0.98;
      } else if (p.type === 'auroraTrail') {
        // Aurora trail particles drift upward with wave motion
        p.x += p.vx;
        p.y += p.vy;
        p.wave += p.waveSpeed;
        // No gravity for these - they rise like aurora rays
  ...
  renderAuroraTrail(p) {
    const { x, y, size, life, color, wave, waveSpeed } = p;
    const alpha = life.toFixed(2);
    
    // Add wave motion to particle movement
    const time = Date.now() * 0.001;
    const waveOffset = Math.sin(time * waveSpeed + wave) * 5;
    ...
    this.ctx.beginPath();
    this.ctx.arc(x + waveOffset, y, size, 0, Math.PI * 2);
    this.ctx.fill();
  ...
  renderAurora(p) {
    const { x, y, width, height, life, color, wavePhase, intensity, curtainIndex, flowSpeed } = p;
    const alpha = (life * intensity * 0.5).toFixed(2); // Much more subtle
    
    // Create flowing vertical curtain like real northern lights
    const time = Date.now() * 0.001;
    
    // Multiple flowing bands within each curtain
    const bandCount = 4;
    for (let band = 0; band < bandCount; band++) {
      const bandOffset = (band * width) / bandCount;
      const bandWidth = width / bandCount * 1.5; // Overlap slightly
      
      // Create smooth vertical gradient
      const gradient = this.ctx.createLinearGradient(
        x + bandOffset, y,
        x + bandOffset, y + height
      );
      
      const bandAlpha = (alpha * (1 - band * 0.15)).toFixed(2);
      
      // Real aurora colors (green lower, purple/pink higher)
      const isGreen = color.includes('0, 255') || color.includes('50, 255');
      
      if (isGreen) {
        gradient.addColorStop(0, `rgba(150, 100, 255, 0)`); 
        gradient.addColorStop(0.2, `rgba(150, 100, 255, ${(bandAlpha * 0.2).toFixed(2)})`);
        gradient.addColorStop(0.5, `${color}${(bandAlpha * 0.6).toFixed(2)})`);
        gradient.addColorStop(0.8, `${color}${(bandAlpha * 0.8).toFixed(2)})`);
        gradient.addColorStop(1, `${color}${(bandAlpha * 0.4).toFixed(2)})`);
      } else {
        gradient.addColorStop(0, `${color}${bandAlpha})`);
        gradient.addColorStop(0.6, `${color}${(bandAlpha * 0.5).toFixed(2)})`);
        gradient.addColorStop(1, `rgba(0, 255, 100, 0)`);
      }
      
      this.ctx.fillStyle = gradient;
      this.ctx.globalAlpha = 0.7; // Make it more transparent
      this.ctx.beginPath();
      
      // Create smooth flowing curtain with gentle waves
      const segments = 30;
      const waveAmplitude = 8 + Math.sin(time * 0.2 + curtainIndex) * 5;
      const currentFlowSpeed = flowSpeed || 0.02;
      
      // Draw one side of the curtain
      for (let i = 0; i <= segments; i++) {
        const segmentY = y + (i / segments) * height;
        const wave = Math.sin(segmentY * 0.005 + time * currentFlowSpeed + wavePhase + band * 0.3) * waveAmplitude;
        const curtainX = x + bandOffset + wave;
        
        if (i === 0) {
          this.ctx.moveTo(curtainX - bandWidth/2, segmentY);
        } else {
          this.ctx.lineTo(curtainX - bandWidth/2, segmentY);
        }
      }
      
      // Draw the other side back up
  ...
  triggerBeatEffect(intensity) {
    ...
    if (intensity > 0.7) {
      switch(this.currentTheme) {
        ...
        case 'hyperAurora':
          this.spawnAuroraWave();
          this.spawnAuroraParticleTrails(); // New particle trails
          if (isSuperPeak) {
            this.auroraPillarExplosion();
            this.coronaBorealisHalo();
            this.triggerAuroraFlare(); // New flare effect
          }
  ...
  // Chromatic Chaos Theme Effects - RGB Split Storm
  rgbSplitStorm(beatStrength) {
    const filter = document.getElementById('rgb-split-filter');
    if (filter && beatStrength) {
      const intensity = beatStrength * 2;
      const redShift = filter.querySelector('feOffset[result="redshift"]');
      const greenShift = filter.querySelector('feOffset[result="greenshift"]');
      
      if (redShift && greenShift) {
        redShift.setAttribute('dx', Math.random() * intensity * 4 - 2);
        greenShift.setAttribute('dx', Math.random() * intensity * -4 + 2);
      }
    }
  }

  // Color Channel Tearing
  colorChannelTearing() {
    ...
    const tearAmount = 5 + Math.random() * 10;
      ...
      for (let x = 0; x < width; x++) {
        ...
        if (Math.random() < 0.3) {
          if (direction === 'horizontal') {
            data[index] = data[(index + tearAmount * 4) % (width * 4)] || data[index];
          } else {
            data[index + 1] = data[(index + tearAmount * width * 4) % data.length] || data[index + 1];
          }
        }
      }
    }
  ...
  chromaticFeedbackLoop(beatStrength) {
    if (!this.canvas || beatStrength < 1.5) return;
    ...
    ctx.globalAlpha = 0.8;
    ctx.filter = 'hue-rotate(15deg) brightness(1.1)';
    ctx.drawImage(tempCanvas, width/2 * 0.05, height/2 * 0.05, width * 0.9, height * 0.9, 0, 0, width, height);
    ctx.globalAlpha = 1.0;
    ctx.filter = 'none';
  }

  // Spawn Color Shift particles
  spawnColorShift(count) {
    for (let i = 0; i < count; i++) {
      this.particles.push({
        x: Math.random() * this.canvas.width,
        y: Math.random() * this.canvas.height,
