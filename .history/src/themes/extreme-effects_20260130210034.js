...
class ExtremeVisualEffects {
  ...
  startNorthernLights() {
    // Create aurora bands across the screen
  ...
  spawnAuroraWave() {
    ...
    const waveCount = 15 + Math.floor(Math.random() * 10);
    ...
    for (let i = 0; i < waveCount; i++) {
      ...
      const particle = {
        ...
        y: startY + Math.sin(progress * Math.PI * 3 + wavePhase) * 50,
  ...
  updateParticles() {
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      if (p.type === 'aurora') {
        // Aurora particles move in waves
        p.x += p.vx * 0.5;
        p.y += p.vy * 0.3;
  ...
  renderAurora(p) {
    const { x, y, size, life, color, waveOffset } = p;
    const alpha = (life * 0.7).toFixed(2);
    
    // Multi-layered aurora effect
    const time = Date.now() * 0.001;
    const waveX1 = x + Math.sin(time + waveOffset) * 40;
    const waveY1 = y + Math.cos(time * 0.8 + waveOffset) * 15;
    const waveX2 = x + Math.sin(time * 1.2 + waveOffset + 1) * 30;
    const waveY2 = y + Math.cos(time * 0.6 + waveOffset + 1) * 20;
    
    // Aurora curtain effect - vertical flowing bands
    const gradient1 = this.ctx.createLinearGradient(waveX1, waveY1 - size, waveX1, waveY1 + size * 2);
    gradient1.addColorStop(0, `${color}0)`);
    gradient1.addColorStop(0.2, `${color}${(alpha * 0.3).toFixed(2)})`);
    gradient1.addColorStop(0.5, `${color}${alpha})`);
    gradient1.addColorStop(0.8, `${color}${(alpha * 0.3).toFixed(2)})`);
    gradient1.addColorStop(1, `${color}0)`);
    
    // Main aurora band
    this.ctx.fillStyle = gradient1;
    this.ctx.fillRect(waveX1 - size/4, waveY1 - size, size/2, size * 2);
    
    // Secondary shimmer layer
    const gradient2 = this.ctx.createLinearGradient(waveX2, waveY2 - size/2, waveX2, waveY2 + size);
    gradient2.addColorStop(0, `${color}0)`);
    gradient2.addColorStop(0.5, `${color}${(alpha * 0.5).toFixed(2)})`);
    gradient2.addColorStop(1, `${color}0)`);
    
    this.ctx.fillStyle = gradient2;
    this.ctx.fillRect(waveX2 - size/6, waveY2 - size/2, size/3, size);
    ...
    const streakGradient = this.ctx.createLinearGradient(waveX1 - size, waveY1, waveX1 + size, waveY1);
    streakGradient.addColorStop(0, `${color}0)`);
    streakGradient.addColorStop(0.5, `${color}${(alpha * 0.6).toFixed(2)})`);
    streakGradient.addColorStop(1, `${color}0)`);
    
    this.ctx.fillStyle = streakGradient;
    this.ctx.fillRect(waveX1 - size, waveY1 - 2, size * 2, 4);
