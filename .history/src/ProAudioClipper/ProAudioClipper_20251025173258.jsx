...
const ProAudioClipper = ({ onNavigate }) => {
  ...
  useKeyboardShortcuts({
    ...
    onPreview: previewSelection, // P key
    onClearSelection: clearSelection, // Escape key
    onMarkIn: () => {
      if (currentTime !== null) {
        ...
        <div className="center-panel">
          ...
          {(selectionStart !== null || selectionEnd !== null) && (
            <div className="selection-controls">
                ...
                {selectionStart !== null && selectionEnd !== null && (
                  <>
                    <span className="selection-time">
                      {selectionStart.toFixed(3)}s - {selectionEnd.toFixed(3)}s 
                      ({(selectionEnd - selectionStart).toFixed(3)}s)
                    </span>
                    <button 
                      className="preview-btn"
                      onClick={previewSelection}
                      title="Preview Selection (P)"
                    >
                      ▶ Preview
                    </button>
                  </>
              ...
              <div className="selection-adjust">
                <div className="adjust-group">
                  <label>In Point:</label>
                  <button onClick={() => adjustSelectionStart(-0.1)} title="Move in point left 0.1s">◀</button>
                  <button onClick={() => adjustSelectionStart(-0.01)} title="Move in point left 0.01s">◂</button>
                  <button onClick={() => adjustSelectionStart(0.01)} title="Move in point right 0.01s">▸</button>
                  <button onClick={() => adjustSelectionStart(0.1)} title="Move in point right 0.1s">▶</button>
                </div>
                
                <div className="adjust-group">
                  <label>Out Point:</label>
                  <button onClick={() => adjustSelectionEnd(-0.1)} title="Move out point left 0.1s">◀</button>
                  <button onClick={() => adjustSelectionEnd(-0.01)} title="Move out point left 0.01s">◂</button>
                  <button onClick={() => adjustSelectionEnd(0.01)} title="Move out point right 0.01s">▸</button>
                  <button onClick={() => adjustSelectionEnd(0.1)} title="Move out point right 0.1s">▶</button>
                </div>
