...
const ProAudioClipper = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    // Always update duration, even if it's the same value (to trigger re-render)
    setDuration(maxDuration);
    
    // Force timeline re-render after duration update
    if (timelineRef.current && maxDuration > 0) {
      setTimeout(() => {
        console.log('Forcing timeline redraw with duration:', maxDuration);
      }, 100);
    }
  }, [trackManager.tracks, multiTrackEngine, timelineRef]);
