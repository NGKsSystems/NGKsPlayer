...
const ProAudioClipper = ({ onNavigate }) => {
  ...
  // Auto-follow playback (keep playhead visible) - Enhanced for per-track speeds
  useEffect(() => {
    if (isPlaying) {
      ...
      if (hasVariableSpeed) {
        ...
        if (currentTime > viewportStart + viewportDuration - conservativeMargin ||
          ...
          setViewportStart(currentTime - viewportDuration * 0.3); // Center playhead more towards left
        }
      } else {
  ...
  }, [isPlaying, currentTime, viewportStart, zoomLevel, trackManager.tracks]);
