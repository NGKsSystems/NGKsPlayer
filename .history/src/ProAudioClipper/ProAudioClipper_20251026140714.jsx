...
const ProAudioClipper = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    console.log('🎵 Tracks changed, updating audio engine:', {
      trackCount: trackManager.tracks.length,
      tracksData: trackManager.tracks.map(t => ({
        id: t.id,
        name: t.name,
        clipCount: t.clips?.length || 0,
        clips: t.clips?.map(c => ({ id: c.id, startTime: c.startTime, endTime: c.endTime })) || []
      }))
    });
    
    multiTrackEngine.updatePlaybackParameters(trackManager.tracks);
  }, [multiTrackEngine, trackManager.tracks]);
  ...
  useEffect(() => {
    ...
    console.log('Duration update:', {
      trackCount: trackManager.tracks.length,
      calculatedMaxDuration: maxDuration,
      tracks: trackManager.tracks.map(t => ({
        name: t.name,
        audioBufferDuration: t.audioBuffer?.duration || 0,
  ...
  const handleZoom = useCallback((newZoomLevel) => {
    ...
    console.log('Zoom sync:', {
      oldZoom: zoomLevel,
      newZoom: clampedZoom,
      oldViewportStart: viewportStart,
      newViewportStart,
      viewportCenter,
      duration
    });
  ...
  const handleClipMove = useCallback((clipId, fromTrackId, toTrackId, updatedClip) => {
    ...
    console.log('🎬 Moving clip:', {
      ...
      from: fromTrackId,
      to: toTrackId,
      oldPosition: { startTime: clipData.clip.startTime, endTime: clipData.clip.endTime },
      newPosition: { startTime: updatedClip.startTime, endTime: updatedClip.endTime }
    });
