...
export const useMultiTrackAudioEngine = () => {
  ...
  const updateTrackParameters = useCallback((tracks) => {
    ...
    tracks.forEach(track => {
      ...
      trackChain.panNode.pan.value = track.pan;
    });
  }, []);
  ...
  const playTracks = useCallback((tracks, startTime = 0, playbackRate = 1) => {
    ...
    try {
      // Create source nodes for all tracks with audio
      tracks.forEach(track => {
        if (track.audioBuffer) {
          ...
          if (trackChain) {
            ...
            // Handle ended event (only need to handle it once)
            if (track.id === tracks[0].id) {
              trackChain.sourceNode.onended = () => {
                if (isPlayingRef.current) {
                  stopTracks();
                  if (onTimeUpdateRef.current) {
                    onTimeUpdateRef.current(track.audioBuffer.duration);
                  }
                }
              };
            }
      ...
      updateIntervalRef.current = setInterval(() => {
        ...
        // Check if we need to stop (using the longest track duration)
        const maxDuration = Math.max(...tracks
  ...
  const seekTracks = useCallback((tracks, time) => {
    ...
    if (wasPlaying && tracks.some(t => t.audioBuffer)) {
      setTimeout(() => {
        playTracks(tracks, clampedTime, currentRate);
      }, 50);
    }
  }, [playTracks, pauseTracks]);
