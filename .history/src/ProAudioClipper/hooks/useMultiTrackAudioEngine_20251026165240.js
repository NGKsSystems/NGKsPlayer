...
export const useMultiTrackAudioEngine = () => {
  ...
  const playTracks = useCallback((tracks, startTime = 0, playbackRate = 1) => {
    console.log('🎵 AUDIO ENGINE: Starting playback at time:', startTime);
    console.log('📊 TRACK DATA:', tracks.map(t => ({
      name: t.name,
      clips: t.clips?.map(c => ({
        name: c.name,
    ...
    try {
      ...
      trackNodesRef.current.forEach((trackChain) => {
        ...
        if (trackChain.clip) {
          ...
          console.log(`🎬 CLIP "${trackChain.clip.name}":`, {
            playheadTime: startTime,
            clipStartTime: clipStartTime,
            clipEndTime: clipEndTime,
            willPlay: startTime >= clipStartTime && startTime < clipEndTime
          });
          ...
          if (startTime >= clipStartTime && startTime < clipEndTime) {
            ...
            console.log(`▶️ Playing clip immediately with offset: ${offsetIntoClip}s`);
            trackChain.sourceNode.start(startWhen, offsetIntoClip);
          } else if (startTime < clipStartTime) {
            // Start the clip at its proper time (delayed start)
            const delayTime = clipStartTime - startTime;
            console.log(`⏰ Scheduling clip to start in: ${delayTime}s`);
            trackChain.sourceNode.start(startWhen + delayTime, 0);
          } else {
            console.log(`⏭️ Skipping clip (playhead past end)`);
          }
          // If startTime > clipEndTime, don't play this clip at all
        } else {
          // Legacy: direct track audio buffer
          console.log(`🔄 Legacy track playback from: ${startTime}s`);
          trackChain.sourceNode.start(startWhen, startTime);
        }
