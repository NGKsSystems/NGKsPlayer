...
export const useTrackManager = () => {
  ...
  const moveClipToTrack = useCallback((clipId, fromTrackId, toTrackId, updatedClip = null) => {
    console.log('🚚 TrackManager: Moving clip between tracks:', {
      clipId, fromTrackId, toTrackId, updatedClip
    });
    ...
    setTracks(prev => {
      console.log('📤 Before removing from source track:', prev.find(t => t.id === fromTrackId));
      
      const result = prev.map(track => {
        if (track.id === fromTrackId) {
          const clip = track.clips.find(c => c.id === clipId);
          if (clip) {
            clipToMove = updatedClip ? { ...updatedClip, trackId: toTrackId } : { ...clip, trackId: toTrackId };
            console.log('📋 Clip to move:', clipToMove);
      ...
      console.log('📤 After removing from source track:', result.find(t => t.id === fromTrackId));
      return result;
    });
    ...
    if (clipToMove) {
      setTracks(prev => {
        console.log('📥 Before adding to destination track:', prev.find(t => t.id === toTrackId));
        ...
        console.log('📥 After adding to destination track:', result.find(t => t.id === toTrackId));
        console.log('🎯 Final tracks state:', result);
        return result;
      });
    }
    ...
    setTracks(prev => prev.map(track => ({
      ...
      clips: track.clips.flatMap(clip => {
        ...
        const secondClip = {
          ...
          audioOffset: firstClipDuration, // Offset into the original audio
        };
