...
export const useAudioEngine = (audioRef) => {
  ...
  const updateIntervalRef = useRef(null); // Track time update intervals
  ...
  const seek = useCallback((time) => {
    ...
    console.log('Seeking to:', time, 'wasPlaying:', wasPlaying); // Debug log
    ...
    // Set new position
    const clampedTime = Math.max(0, Math.min(time, audioBufferRef.current?.duration || 0));
    pausedPositionRef.current = clampedTime;
    
    // Immediately update the time display - this is critical for visual feedback
    if (onTimeUpdateRef.current) {
      onTimeUpdateRef.current(clampedTime);
    }
    
    // Resume playback if it was playing (with a small delay to avoid conflicts)
    if (wasPlaying) {
      setTimeout(() => {
        if (audioBufferRef.current) { // Extra safety check
          play(clampedTime, currentRate);
        }
