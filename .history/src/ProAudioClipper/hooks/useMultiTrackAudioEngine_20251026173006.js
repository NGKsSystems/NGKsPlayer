          ...
          track.clips.forEach(clip => {
            if (clip.audioBuffer) {
              ...
              if (trackChain) {
                ...
                console.log(`Created trackChain for clip ${clip.id} with audioOffset: ${clip.audioOffset || 0}`);
                
                // Add to trackNodes array for playback
                trackNodesRef.current.push(trackChain);
              }
            }
          });
        }
      ...
      trackNodesRef.current.forEach((trackChain) => {
        ...
        if (trackChain.clip) {
          ...
          if (startTime >= clipStartTime && startTime < clipEndTime) {
            ...
            const totalOffset = offsetIntoClip + (trackChain.clip.audioOffset || 0);
            console.log(`Playing clip ${trackChain.clip.id} with audioOffset: ${trackChain.clip.audioOffset || 0}, totalOffset: ${totalOffset}`);
            ...
            console.log(`Playing delayed clip ${trackChain.clip.id} with audioOffset: ${audioOffset}`);
