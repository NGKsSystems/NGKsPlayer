      ...
      <div className="main-layout">
        ...
        <div className="center-panel">
          ...
          <div style={{
            ...
            <div>Tracks: {trackManager.tracks.length}</div>
            <div>Active Track: {trackManager.activeTrackId}</div>
            <div>Current Time: {currentTime.toFixed(2)}s</div>
            <div>Duration: {duration.toFixed(2)}s</div>
            <div>Zoom: {zoomLevel.toFixed(1)}x</div>
            <div>Tool: {selectedTool}</div>
            {trackManager.tracks.map((track, i) => (
              <div key={track.id} style={{ fontSize: '10px', marginTop: '4px' }}>
                Track {i+1}: {track.name} | Clips: {track.clips?.length || 0} | Audio: {track.audioBuffer ? 'Yes' : 'No'}
              </div>
            ))}
          </div>
