...
const SimpleWaveform = ({ 
  ...
  useEffect(() => {
    ...
    // Calculate margins (1.25% on each side = 2.5% total)
    const marginPercent = 0.0125; // 1.25%
    const marginWidth = width * marginPercent;
    const effectiveWidth = width - (2 * marginWidth);
    
    // Calculate samples per pixel for the effective width (no playback rate adjustment here)
    const samplesPerPixel = relevantSamples / effectiveWidth;
