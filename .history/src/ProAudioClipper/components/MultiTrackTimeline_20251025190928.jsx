...
const MultiTrackTimeline = React.forwardRef(({
  ...
  const renderTimeline = useCallback(() => {
    ...
    if (!canvas) {
      console.log('MultiTrackTimeline: No canvas ref');
      return;
    }
    ...
    // Debug logging
    console.log('MultiTrackTimeline render:', {
      tracks: tracks.length,
      canvasSize: { width: rect.width, height: rect.height },
      timelineHeight,
      tracksData: tracks.map(t => ({ id: t.id, name: t.name, clips: t.clips?.length || 0, hasAudio: !!t.audioBuffer }))
    });
    ...
    tracks.forEach((track, index) => {
      try {
        ...
        console.log(`Rendered track ${index}: ${track.name}, clips: ${track.clips?.length || 0}, hasAudio: ${!!track.audioBuffer}`);
      } catch (trackError) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        Tracks: {tracks.length} | Tool: {selectedTool} | H: {timelineHeight}px
      </div>
      ...
      <canvas
        ...
        style={{
          ...
          height: '100%',
          display: 'block',
          background: '#222' // Debug background for canvas
        }}
      />
    </div>
  );
