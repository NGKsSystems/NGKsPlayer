...
const TimelineRuler = ({ 
  ...
  // Debug logging for timeline scaling
  useEffect(() => {
    console.log('TimelineRuler scaling:', {
      duration,
      pixelsPerSecond,
      zoom,
      timelineWidth,
      ...
      playheadPosition,
      viewportStart,
      durationSource: duration === 0 ? 'DEFAULT VALUE (0)' : 'PASSED PROP'
    });
  }, [duration, pixelsPerSecond, zoom, timelineWidth, currentTime, playheadPosition, viewportStart]);
  ...
  const generateTicks = useCallback(() => {
    const ticks = [];
    if (!rulerRef.current) return ticks;
