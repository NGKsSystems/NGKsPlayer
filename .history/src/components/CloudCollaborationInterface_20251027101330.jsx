    ...
    useEffect(() => {
        ...
        const initializeCloud = async () => {
            try {
                ...
                setSecurityDashboard(cloud.getSecurityDashboard());
            ...
            <div className="bg-gray-800 rounded-lg p-4">
                ...
                <div className="space-y-3">
                    <div className="flex items-center space-x-3">
                        ...
                        />
                        <label htmlFor="auth-password" className="text-gray-300">
                    ...
                    <div className="flex items-center space-x-3">
                        <input
                            ...
                            name="authMethod"
                            value="mfa"
                            checked={authMethod === 'mfa'}
                            onChange={(e) => setAuthMethod(e.target.value)}
                        />
                        <label htmlFor="auth-mfa" className="text-gray-300">
                    ...
                    <div className="flex items-center space-x-3">
                        <input
                            ...
                            onChange={(e) => setAuthMethod(e.target.value)}
                        />
                        <label htmlFor="auth-oauth" className="text-gray-300">
                ...
                {securityDashboard?.metrics && (
                    <div className="space-y-3">
                        <div className="flex justify-between">
                            <span className="text-gray-300">Security Events (24h):</span>
                            <span className="text-white">{securityDashboard.metrics.events.last24Hours}</span>
                        </div>
                        
                        <div className="flex justify-between">
                            <span className="text-gray-300">Active Alerts:</span>
                            <span className={`${securityDashboard.metrics.alerts.unacknowledged > 0 ? 'text-red-400' : 'text-green-400'}`}>
                                {securityDashboard.metrics.alerts.unacknowledged}
                            </span>
                        </div>
                        
                        <div className="flex justify-between">
                            <span className="text-gray-300">High Threats (24h):</span>
                            <span className={`${securityDashboard.metrics.threats.high > 0 ? 'text-red-400' : 'text-green-400'}`}>
                                {securityDashboard.metrics.threats.high}
                            </span>
                        </div>
                    </div>
                )}
            ...
            <div className="bg-gray-800 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button
                        onClick={() => {
                            if (cloudSystemRef.current) {
                                const audit = cloudSystemRef.current.exportSecurityAudit();
                    ...
                    <button
                        onClick={() => {
                            if (cloudSystemRef.current) {
                                setSecurityDashboard(cloudSystemRef.current.getSecurityDashboard());
                            }
                        }}
                    ...
                    <button
                        onClick={() => {
                            if (cloudSystemRef.current) {
                                cloudSystemRef.current.monitor.alerts.forEach(alert => {
                                    if (!alert.acknowledged) {
                                        cloudSystemRef.current.monitor.acknowledgeAlert(alert.id);
                                    }
                                });
                                setSecurityDashboard(cloudSystemRef.current.getSecurityDashboard());
                            }
                    ...
                    <button
                        onClick={() => {
                            alert('Security scan would be implemented with external security tools');
                        }}
