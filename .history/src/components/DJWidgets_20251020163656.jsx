...
export const MixerWidget = ({ deckAudioRefs }) => {
  const [mixerControls, setMixerControls] = useState({
    crossfader: { x: 10, y: 50, width: 180, height: 50, minimized: false },
    // Volume Controls
    volumeLeft: { x: 140, y: 50, width: 80, height: 200, minimized: false },
    volumeRight: { x: 540, y: 60, width: 80, height: 200, minimized: false },
    gainA: { x: 60, y: 180, width: 100, height: 100, minimized: false },
    gainB: { x: 440, y: 180, width: 100, height: 100, minimized: false },
    reverbA: { x: 200, y: 120, width: 75, height: 60, minimized: false },
    reverbB: { x: 285, y: 120, width: 75, height: 60, minimized: false },
    filterA: { x: 200, y: 190, width: 75, height: 60, minimized: false },
    filterB: { x: 285, y: 190, width: 75, height: 60, minimized: false },
    micInput: { x: 10, y: 220, width: 85, height: 60, minimized: false },
    micGain: { x: 105, y: 220, width: 85, height: 60, minimized: false },
    masterVol: { x: 370, y: 50, width: 75, height: 120, minimized: false },
    cueVol: { x: 370, y: 180, width: 75, height: 80, minimized: false },
    // Moved deck controls with A/B switches
    pitchA: { x: 450, y: 50, width: 80, height: 120, minimized: false },
    pitchB: { x: 540, y: 50, width: 80, height: 120, minimized: false },
    loopControls: { x: 10, y: 290, width: 150, height: 80, minimized: false },
    cuePoints: { x: 170, y: 290, width: 180, height: 80, minimized: false },
    syncControls: { x: 360, y: 290, width: 100, height: 60, minimized: false },
    deckSettings: { x: 470, y: 290, width: 120, height: 80, minimized: false },
    performanceControls: { x: 450, y: 140, width: 140, height: 80, minimized: false }
  });
  ...
  // Force correct positioning on component mount
  useEffect(() => {
    const resetPositions = () => {
      ...
      // Set React state for immediate UI update
      setMixerControls(prev => ({
        ...prev,
        ...defaults
      }));
      
      // ALSO set localStorage so widgets load with correct positions
      try {
        ...
        localStorage.setItem('widget-ampRight-position', JSON.stringify({ x: 440, y: 180 }));
        localStorage.setItem('widget-ampRight-size', JSON.stringify({ width: 100, height: 100 }));
        console.log('✅ Reset: localStorage updated with consistent positions');
      } catch (e) {
        console.warn('Failed to set localStorage during reset:', e);
      }
    };
    
    // Remove the localStorage clearing that was causing issues
    
    // Reset immediately
    resetPositions();
    
    // Also reset after a small delay to override any async loading
    setTimeout(resetPositions, 100);
    setTimeout(resetPositions, 500);
    
    // Make reset function available globally for manual clearing
    ...
    <div className="mixer-content">
      <div className="mixer-workspace">
        ...
        <DraggableWidget
          id="volumeLeft"
          title="Volume A"
          {...mixerControls.volumeLeft}
          mirrorSizeWith="volumeRight"
          mirrorSizeOnly={true}
          onUpdate={(updates) => {
            // Debug Volume A changes that might affect Volume B
            if (updates.width !== undefined || updates.height !== undefined) {
              console.log('🔍 VOLUME A SIZE CHANGE (may affect Volume B):', { 
                width: updates.width, 
            ...
            if (updates.x !== undefined || updates.y !== undefined) {
              console.log('🔍 VOLUME A POSITION CHANGE:', { 
                x: updates.x, 
                y: updates.y, 
                timestamp: new Date().toLocaleTimeString()
              });
            }
            handleControlUpdate('volumeLeft', updates);
          }}
          onMinimize={() => handleControlMinimize('volumeLeft')}
        ...
        <DraggableWidget
          id="volumeRight"
          title="Volume B"
          {...mixerControls.volumeRight}
          mirrorSizeWith="volumeLeft"
          mirrorSizeOnly={true}
          onUpdate={(updates) => {
            // Only log if position changes (not size)
            if (updates.x !== undefined || updates.y !== undefined) {
              console.log('� silenced'); // REMOVED
            }
            handleControlUpdate('volumeRight', updates);
          }}
          onMinimize={() => handleControlMinimize('volumeRight')}
          className="volume-widget"
          style={{ position: 'absolute', zIndex: 1000 }}
        >
          <div className="volume-control vertical">
...
window.resetToConsistentLayout = () => {
  ...
  window.dispatchEvent(new CustomEvent('forceWidgetRefresh'));
};