...
export default function RequestQueue({ onClose, onLoadTrack }) {
  ...
  // Load payment handles on mount
  useEffect(() => {
    const loadPaymentHandles = async () => {
      const result = await window.api.invoke('request-server:getPaymentHandles');
      if (result.success && result.handles) {
        setPaymentHandles(result.handles);
      }
    };
    loadPaymentHandles();
  }, []);
