...
export default function RequestQueue({ onClose, onLoadTrack }) {
  ...
  useEffect(() => {
    ...
    if (window.electronAPI?.receive) {
      console.log('[RequestQueue] Setting up listener for request-server:newRequest');
  ...
  const checkServerStatus = async () => {
    try {
      ...
      // Also load existing requests if server is running
      if (status.isRunning && status.requestCount > 0) {
        console.log('[RequestQueue] Server has existing requests, fetching...');
        // We'll need to add an IPC handler to get existing requests
      }
