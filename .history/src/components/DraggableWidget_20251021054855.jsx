...
const DraggableWidget = ({ 
  ...
  const getAllWidgetPositions = useCallback(() => {
    ...
    // Get all major widgets
    const allWidgets = document.querySelectorAll('.draggable-widget[data-widget-id]');
    allWidgets.forEach(widget => {
      ...
      if (widgetId) {
        ...
        widgets[widgetId] = {
          ...
          width: parseInt(widget.style.width) || rect.width,
          height: parseInt(widget.style.height) || rect.height,
          minimized: isMinimized
        };
      }
    });
    
    // Get all sub-widgets (transport, VU meters, etc.)
    const subWidgets = document.querySelectorAll('.deck-sub-widget, .mixer-sub-widget');
    subWidgets.forEach(widget => {
      let widgetId = widget.getAttribute('data-widget-id');
      
      // Generate ID for sub-widgets if they don't have one
      if (!widgetId) {
        const parentWidget = widget.closest('[data-widget-id]');
        const parentId = parentWidget?.getAttribute('data-widget-id') || 'unknown';
        const classList = widget.className.split(' ');
        const widgetType = classList.find(cls => 
          cls.includes('transport') || cls.includes('vu') || cls.includes('waveform') || 
          cls.includes('eq') || cls.includes('library') || cls.includes('mixer') ||
          cls.includes('jog') || cls.includes('pitch')
        ) || 'sub';
        widgetId = `${parentId}-${widgetType}`;
        widget.setAttribute('data-widget-id', widgetId);
      }
      
      const rect = widget.getBoundingClientRect();
      const parentRect = widget.offsetParent?.getBoundingClientRect();
      
      widgets[widgetId] = {
        x: widget.offsetLeft || 0,
        y: widget.offsetTop || 0,
        width: rect.width,
        height: rect.height,
        isSubWidget: true,
    ...
    console.log(`Collected ${Object.keys(widgets).length} widget positions:`, Object.keys(widgets));
    return widgets;
  }, []);
  ...
  }, [layoutManager, getAllWidgetPositions]);

  const handleSaveAsLayout = useCallback((newName) => {
    if (!layoutManager) return;
    
    const allPositions = getAllWidgetPositions();
    layoutManager.saveAsLayout(newName, allPositions);
  }, [layoutManager, getAllWidgetPositions]);
  ...
  }, [layoutManager, id, getAllWidgetPositions]);
  ...
  const handleMouseDown = useCallback((e) => {
    // Skip blocking for volume controls - they need to be draggable
    const isVolumeWidget = e.target.closest('.volume-widget-draggable');
