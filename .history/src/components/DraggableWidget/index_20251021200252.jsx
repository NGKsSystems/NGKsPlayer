...
const DraggableWidget = ({ 
  ...
  // Auto-save position changes for major widgets
  useEffect(() => {
    if (isMajorWidget && layoutManager) {
      // Debounce auto-save to avoid excessive saves during dragging
  ...
  }, [position, size, isMinimized, isMajorWidget, layoutManager, getAllWidgetPositions]);
  ...
  const handleLoadLayout = useCallback((layoutName) => {
    ...
    if (widgets[id]) {
      ...
      console.log(`🔄 Updated ${id} state:`, widgetData);
    }
  }, [id, layoutManager]);
