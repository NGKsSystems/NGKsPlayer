...
const Sampler = ({ 
    ...
    const loadSamples = async () => {
      try {
        ...
        if (savedSamples) {
          ...
          // Check if we have old format with Base64 data or new format with just metadata
          if (parsed.length > 0 && parsed[0].dataBase64) {
            // Old format - try to decode (but this might fail due to size)
            const samplesWithData = parsed.map(sample => {
              try {
                if (!sample.dataBase64) return null;
                ...
                console.warn('Skipping corrupted sample:', sample.name);
                return null;
              }
            }).filter(Boolean);
            ...
            // New format - just metadata, user will need to reload samples
            console.log('Found sample metadata for', parsed.length, 'samples');
            // Don't load - user needs to re-add samples
          }
        }
      } catch (error) {
        console.warn('Could not load saved samples:', error.message);
  ...
  useEffect(() => {
    ...
    try {
      // Only save sample metadata, not the actual audio data (too large for localStorage)
