    ...
    <div style={{
      ...
      <div style={{
        ...
        <button
          onClick={() => {
            ...
            if (container) {
              container.scrollBy({ left: -(PAD_WIDTH + 8) * 5, behavior: 'smooth' });
            }
          }}
          style={{
            background: 'linear-gradient(145deg, #3498db, #2980b9)',
        ...
        <div style={{ 
          ...
          Pads {Math.floor(scrollPosition / (PAD_WIDTH + 8)) + 1}-{Math.min(Math.floor(scrollPosition / (PAD_WIDTH + 8)) + VISIBLE_PADS, totalPadsNeeded)} of {totalPadsNeeded}
        </div>
      ...
      >
        ...
        <div style={{
          ...
          {renderPads.slice(0, Math.ceil(renderPads.length / 2)).map((sample, index) => (
            <SamplePad
        ...
        <div style={{
          ...
          {renderPads.slice(Math.ceil(renderPads.length / 2)).map((sample, index) => {
            const actualIndex = Math.ceil(renderPads.length / 2) + index;
            return (
              <SamplePad
