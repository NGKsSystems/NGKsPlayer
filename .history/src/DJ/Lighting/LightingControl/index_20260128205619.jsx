...
const LightingControl = ({ id, onStyleChange = () => {}, style = {}, audioManager, ...props }) => {
  ...
  const scanDevices = async () => {
    console.log('[LightingControl] 🔍 Scanning for DMX interfaces...');
  ...
  const handleConnect = async () => {
    if (connectionSettings.connected) {
      ...
      try {
        ...
        console.log('[LightingControl] ✓ Connected to DMX successfully');
...
1. 🎛️ Hardware Controller (SoundSwitch - OPTIONAL)
   ↓ USB (optional)
   
2. 💻 Laptop - "THE BRAIN"
   Your laptop running this software
   ↓ USB Cable (Circled in pic)
   
3. 📦 DMX Interface - "THE TRANSLATOR"
   Small DMX interface box (where lights actually connect)
   ↓ DMX Cables (Daisy-chain)
...
• Enttec Open DMX USB ($65) - Simple, single universe
• DMXKing ultraDMX Micro ($89) - Compact, reliable
• Enttec DMX USB Pro Mk2 ($299) - Professional, 2 universes
...
2. DMX Interface ($65-300) ← Lights plug into this
...
• USB DMX Interface - Most common, plug and play
• Art-Net (Ethernet) - Network-based, supports multiple universes
• sACN / E1.31 (Ethernet) - Industry standard network protocol
• WiFi DMX - Wireless connection
          ...
          <div className="modal-overlay" onClick={() => setShowHardwareGuide(false)}>
            <div className="hardware-modal" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <h3>💡 DMX Hardware Setup Guide</h3>
