...
const EQB = ({ audioContext, gainNode, pannerNode, style, onPositionChange, onSizeChange, ...props }) => {
  ...
  useEffect(() => {
    ...
    try {
      console.log('[EQB] Initializing EQ filters...');
      ...
      setTimeout(() => {
        if (window.audioManagerRef?.current) {
          ...
          console.log(`[EQB] EQ chain inserted for Deck B`);
        }
      }, 500);
      ...
      console.log(`[EQB] Created ${filters.length} EQ filters for Deck B`);
  ...
  const handleGainChange = (index, value) => {
    ...
    if (filtersRef.current[index] && audioContext) {
      try {
        ...
        console.log(`[EQB] Band ${index} (${expandedFrequencies[index]}Hz) set to ${gainValue}dB`);
      } catch (error) {
  ...
  const handlePresetChange = (presetName) => {
    ...
    if (filtersRef.current.length > 0 && audioContext) {
      ...
      console.log(`[EQB] Applied preset: ${presetName}`);
    }
  };
  ...
  const handleBypass = () => {
    ...
    if (filtersRef.current.length > 0 && audioContext) {
      ...
      console.log(`[EQB] EQ ${newBypassed ? 'bypassed' : 'active'}`);
    }
  };
  ...
  const handleReset = () => {
    ...
    console.log(`[EQB] EQ reset to flat`);
  };
  ...
  return (
    <div 
      ref={widgetRef}
      className={`eq-b-widget ${isDragging ? 'dragging' : ''} ${isResizing ? 'resizing' : ''}`} 
      style={style} 
      {...props}
    >
