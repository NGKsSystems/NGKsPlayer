...
const EQA = ({ audioContext, gainNode, pannerNode, style, onPositionChange, onSizeChange, ...props }) => {
    ...
    if (audioContext && gainNode && pannerNode && eqFilters.length === 0) {
      ...
      setTimeout(() => {
        if (window.audioManagerRef?.current) {
          ...
          console.log(`[EQA] EQ chain inserted for Deck A`);
        }
      }, 500);
  ...
  useEffect(() => {
    if (eqFilters.length > 0 && audioContext) {
      ...
      console.log(`[EQA] EQ ${bypassEQ ? 'bypassed' : 'active'}`);
    }
  ...
  const handleBandChange = useCallback((bandIndex, value) => {
    ...
    if (eqFilters[bandIndex] && audioContext) {
      try {
        ...
        console.log(`[EQA] Band ${bandIndex} (${frequencyValues[bandIndex]}Hz) set to ${gainValue}dB`);
      } catch (error) {
        console.error(`[EQA] Failed to update band ${bandIndex}:`, error);
      }
    }
  }, [eqValues, eqFilters, audioContext, bypassEQ, frequencyValues]);
  ...
  const handlePresetChange = useCallback((presetName) => {
    ...
    if (eqFilters.length > 0 && audioContext) {
      ...
      console.log(`[EQA] Applied preset: ${presetName}`);
    }
  ...
  const handleReset = useCallback(() => {
    ...
    if (eqFilters.length > 0 && audioContext) {
      ...
      console.log(`[EQA] EQ reset to flat`);
    }
