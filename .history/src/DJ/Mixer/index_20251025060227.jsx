...
const Mixer = ({ audioManager, mixerLayout = {}, onMixerLayoutChange = () => {}, ...props }) => {
  console.log('🎛️ Mixer component received mixerLayout:', mixerLayout);
  ...
  const handleMixerSubwidgetUpdate = (updates) => {
    ...
    if (KNOB_WIDGET_IDS.includes(resizedWidgetId) && resizedWidget && (resizedWidget.width || resizedWidget.height)) {
      ...
      KNOB_WIDGET_IDS.forEach(knobId => {
        // CRITICAL FIX: Only update the size, preserve all other properties from the original update
        if (knobId === resizedWidgetId) {
          // For the widget being resized, use the complete update
          synchronizedUpdates[knobId] = resizedWidget;
        } else {
          // For other knobs, only sync the size, keep existing position
          synchronizedUpdates[knobId] = {
            x: mixerLayout[knobId]?.x,
            y: mixerLayout[knobId]?.y,
            width: resizedWidget.width || mixerLayout[knobId]?.width || 85,
            height: resizedWidget.height || mixerLayout[knobId]?.height || 130
          };
        }
      });
      ...
      // For all other updates (position changes, single widget updates), merge with existing layout
      console.log('🎛️ Sending single widget update:', updates);
    ...
    <div className="mixer-container">
      <div className="mixer-workspace" style={{ position: 'relative', width: '100%', height: '600px' }}>
        ...
        <DraggableWidget
          ...
          onUpdate={(updates) => {
            console.log('🎛️ gain-a widget update:', updates);
            handleMixerSubwidgetUpdate({ 'gain-a': updates });
          }}
        >
          <Knob
            id="gain-a-knob"
