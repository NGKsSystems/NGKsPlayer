...
const Sampler = ({ audioManager }) => {
  ...
  return (
    ...
    <div style={{const Sampler = ({ audioManager }) => {

      background: 'linear-gradient(145deg, #2c3e50, #34495e)',

      border: '2px solid #3498db',  const [samples, setSamples] = useState([]);

      borderRadius: '12px',

      padding: '12px',  const [dragActive, setDragActive] = useState(false);const Sampler = ({ audioManager }) => {

      color: 'white',

      fontFamily: 'Arial, sans-serif',  const [loading, setLoading] = useState(false);

      width: '100%',

      height: '100%',  const [playingId, setPlayingId] = useState(null);  const [samples, setSamples] = useState([]);

      display: 'flex',

      flexDirection: 'column'  const [scrollPosition, setScrollPosition] = useState(0);

    }}>

      <h3 style={{ margin: 0, fontSize: '18px', color: '#3498db' }}>  const fileInputRef = useRef(null);  const [dragActive, setDragActive] = useState(false);const Sampler = ({ audioManager }) => {const Sampler = ({ audioManager }) => {

        Sample Pads - Basic Version

      </h3>  const audioContextRef = useRef(new (window.AudioContext || window.webkitAudioContext)());

      <div style={{ 

        flex: 1,   const sourceNodesRef = useRef({});  const [loading, setLoading] = useState(false);

        display: 'flex', 

        alignItems: 'center',   const scrollContainerRef = useRef(null);

        justifyContent: 'center',

        color: '#888',  const [playingId, setPlayingId] = useState(null);  const [samples, setSamples] = useState([]);  const [samples, setSamples] = useState([]);

        fontSize: '14px'

      }}>  // Constants for the scrolling system

        Horizontal scrolling sample system will be implemented here

      </div>  const MAX_SAMPLE_PADS = 100;  const [scrollPosition, setScrollPosition] = useState(0);
