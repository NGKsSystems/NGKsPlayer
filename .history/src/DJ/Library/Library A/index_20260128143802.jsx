...
const LibraryA = ({ id, deck = 'A', onTrackLoad = () => {}, onTrackPreview = () => {}, onStyleChange = () => {}, tracks = [], isLoading = false, style = {}, ...props }) => {
  ...
  const [libraryState, setLibraryState] = useState(() => {
    try {
      const saved = localStorage.getItem('library-A-state');
      if (saved) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('library-A-state', JSON.stringify(libraryState));
    } catch (error) {}
  }, [libraryState]);

  // Drag handlers
  const handleMouseDown = useCallback((e) => {
    if (e.target.closest('.library-a-header') && !e.target.closest('.search-input, .sort-btn')) {
      setIsDragging(true);
      setDragOffset({
            ...
            filteredTracks.map((track, index) => (
              ...
              >
                ...
                <div className="track-meta">
                  {track.bpm && <span className="track-bpm" title="BPM">♩{Math.round(track.bpm)}</span>}
                  {track.key && <span className="track-key" title="Key">{track.key}{track.mode?.charAt(0) || ''}</span>}
                  {track.loudness && <span className="track-loudness" title="Loudness (0-100)">🔊{track.loudness}</span>}
                  {track.gainRecommendation && <span className="track-gain" title="Gain">{track.gainRecommendation}</span>}
