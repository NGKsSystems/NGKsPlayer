      ...
      <div className="deck-b-content">
        ...
        <div className="fine-tune-section">
          <div className="fine-tune-header">
            ...
            <span className="fine-tune-value">{(position || 0) > 0 ? '+' : ''}{(position || 0).toFixed(2)}s</span>
          </div>
          <input 
            type="range" 
            min={Math.max(0, position - 0.2) || 0}
            max={Math.min(duration, position + 0.2) || 0.4}
            step="0.01"
            value={position || 0}
            onChange={(e) => onSeek(parseFloat(e.target.value))}
            className="fine-tune-slider"
            title="Fine position adjustment for cueing (±0.20 seconds)"
          />
        </div>
      </div>
