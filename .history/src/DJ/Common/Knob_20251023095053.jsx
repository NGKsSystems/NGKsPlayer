...
const Knob = ({ 
  ...
  const getDisplayValue = () => {
    if (controlType === 'gain') {
      ...
      // Safety check - if logFreq is NaN or Infinity, return safe value
      if (!isFinite(logFreq)) {
        return '20Hz';
      }
      
      if (logFreq >= 1000) {
        // Format as kHz without scientific notation
        const khzValue = (logFreq / 1000).toFixed(1);
        // Ensure khzValue is not in scientific notation
        const numValue = parseFloat(khzValue);
        if (!isFinite(numValue)) return '20kHz';
        return `${numValue}k`;
      }
