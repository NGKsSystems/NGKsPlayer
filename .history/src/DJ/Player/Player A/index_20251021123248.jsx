          ...
          <div className="waveform">
            {Array.from({ length: 32 }, (_, i) => (
              <div 
                ...
                style={{ 
                  height: `${Math.max(5, waveformData[i] || Math.random() * 30 + 10)}%`,
                  opacity: isPlaying ? 0.8 : 0.3,
                  backgroundColor: isPlaying ? '#3b82f6' : '#64748b'
                }}
              />
            ))}
          </div>
        ...
        <div className="vu-meter-container">
          ...
          <div className="vu-meter">
            <div 
              ...
              style={{ 
                height: `${vuLevel}%`,
                backgroundColor: vuLevel > 80 ? '#ef4444' : vuLevel > 60 ? '#f59e0b' : '#22c55e'
              }}
            />
            <div className="vu-meter-scale">
              {[0, 20, 40, 60, 80, 100].map(level => (
                <div key={level} className="vu-scale-mark" style={{ bottom: `${level}%` }}>
                  {level === 100 ? '0' : level === 80 ? '-6' : level === 60 ? '-12' : level === 40 ? '-18' : level === 20 ? '-24' : '-∞'}
                </div>
              ))}
            </div>
          </div>
