            ...
            {(() => {
              ...
              if (track?.title) {
                ...
                if (audioManagerTrack?.title) {
                  title = audioManagerTrack.title;
                  artist = audioManagerTrack.artist || '';
                } else if (audioManagerTrack?.fileName) {
                  title = audioManagerTrack.fileName.replace(/\.[^/.]+$/, "");
                } else if (audioManagerTrack?.filePath) {
                ...
                if (audioElement && audioElement.src) {
                  ...
                  if (match) {
                    title = decodeURIComponent(match[0]);
                  } else {
                    title = 'Playing Track';
                  }
                }
                ...
                title = track ? 'Track Loaded (No Title)' : 'No Track Loaded';
              }
              
              // Combine title and artist if both exist
              if (title && artist && artist !== title) {
                return `${artist} - ${title}`;
              }
              return title || 'No Track Loaded';
            })()}
          </div>
