...
const PlayerA = ({ 
  ...
  // Debug effect to log track data
  useEffect(() => {
    console.log('[Player A] Track prop changed:', {
      track,
      hasTrack: !!track,
      title: track?.title,
      fileName: track?.fileName,
      filePath: track?.filePath,
      trackKeys: track ? Object.keys(track) : 'no track'
    });
  }, [track]);
      ...
      <div className="player-a-content">
        ...
        <div className="track-info">
          <div className="track-title" style={{color: 'white', fontSize: '16px'}}>
            {(() => {
              console.log('[Player A] Rendering track title, track prop:', track);
              
              // Fallback: Try to get track from AudioManager if track prop is null
              if (!track && audioManager) {
                const audioManagerTrack = audioManager.getCurrentTrack('A');
                console.log('[Player A] AudioManager getCurrentTrack:', audioManagerTrack);
                if (audioManagerTrack) {
              ...
              if (!track && audioManager) {
                ...
                if (audioElement && audioElement.src) {
                  console.log('[Player A] Extracting from audio src:', audioElement.src);
                  const srcUrl = audioElement.src;
                  // Extract filename from local:// URL or file path
              ...
              if (track.fileName) return track.fileName.replace(/\.[^/.]+$/, ""); // Remove extension
              if (track.filePath) {
