...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const [showBeatControls, setShowBeatControls] = useState(false) // Control panel toggle
  const [beatSpikeThreshold, setBeatSpikeThreshold] = useState(1.4) // 1.0 - 2.0
  const [beatMinimum, setBeatMinimum] = useState(100) // 50 - 200
  const [beatGate, setBeatGate] = useState(250) // 100 - 500ms
  const [beatHistoryLength, setBeatHistoryLength] = useState(60) // 10 - 100
  const [panelPos, setPanelPos] = useState({ x: 20, y: 80 }) // Draggable panel position
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // SPIKE DETECTION: Use adjustable parameters from UI controls
      const bassSpike = bassAverage > avgBass * beatSpikeThreshold
      const absoluteMinimum = bassAverage > beatMinimum
      ...
      if (Math.random() < 0.016) {
        console.log('🎵 Beat Check:', { 
          bassAvg: bassAverage.toFixed(1), 
          avgBass: avgBass.toFixed(1),
          threshold: beatSpikeThreshold, 
          spike: bassSpike, 
          min: absoluteMinimum,
          gate: timeSinceLastBeat > beatGate,
          enabled: beatPulseEnabledRef.current
        })
      }
      
      // Only trigger on sharp transients (drums), ignore vocals/sustained instruments
      if (beatPulseEnabledRef.current && bassSpike && absoluteMinimum && timeSinceLastBeat > beatGate) {
        console.log('💥 BEAT TRIGGERED!', { bass: bassAverage.toFixed(1), avg: avgBass.toFixed(1) })
