...
import Equalizer from '../components/Equalizer/index.jsx'
import LayerRemover from './LayerRemover.jsx'
// import DJDualPlayer from '../components/DJDualPlayer.jsx'
// import DeckQueue from '../components/DeckQueue.jsx'
import AutoDJPanel from '../components/AutoDJPanel/index.jsx'
...
import { pickNextIndex, pickPrevIndex, getCachedNext, setCachedNext, clearCachedNext } from '../utils/randomizer'
...
// Debug helper - default no-op to avoid noisy console output in production
// Debug helper — controlled by localStorage so logs can be enabled in dev
// without changing source. Usage (DevTools):
//   localStorage.setItem('ngks:debugLogs', '1')
const debug = (...args) => {
  try {
    const v = localStorage.getItem('ngks:debugLogs')
    if (v === '1' || v === 'true') console.log('[NowPlaying]', ...args)
  } catch (e) {
    // storage might be restricted — ignore
  }
}
// Silence noisy console.log output for this module by default; use console.error/warn for real problems.
console.log = debug
...
export default function NowPlayingSimple({ onNavigate }) {
  console.log('🔥 [NowPlaying] Component RENDERING - entry point');
  ...
  console.log(`🔥 [NowPlaying] ComponentId: ${componentId.current}, globalInstance: ${globalNowPlayingInstance}`);
  
  // Determine if this should be the active instance immediately
  if (!globalNowPlayingInstance) {
    console.log(`🔥 [NowPlaying] No global instance, this will be ACTIVE`);
    // Clean up any previous global audio elements
    if (globalAudioElements) {
      debug(`🧨 Cleaning up previous global audio elements`);
    ...
    console.log(`🔥 [NowPlaying] Set as ACTIVE instance: ${componentId.current}`);
  } else if (globalNowPlayingInstance !== componentId.current) {
    console.warn(`⚠️  [NowPlaying] INACTIVE INSTANCE! Active: ${globalNowPlayingInstance}, This: ${componentId.current}`);
    console.warn(`⚠️  [NowPlaying] Clearing old instance and taking over`);
    // Force takeover - clear the old instance
    globalNowPlayingInstance = componentId.current;
    isInactiveRef.current = false;
    debug(`🔄 NowPlaying component TOOK OVER, new ID: ${componentId.current}`);
  }

  // CRITICAL: Clean up global reference on unmount to allow new instances
  useEffect(() => {
    console.log(`✅ [NowPlaying] Component mounted and ready to render`);
    return () => {
      if (globalNowPlayingInstance === componentId.current) {
        debug(`🧹 NowPlaying component unmounting, clearing global instance`);
  ...
  const [playMode, setPlayMode] = useFeature('playMode', 'inOrder') // 'inOrder' | 'shuffle' | 'randomNoRepeat' | 'repeatAll'
  ...
  useEffect(() => { setCachedNextIndex(null); setNextTrackInfo(null); try { clearCachedNext(queue) } catch (e) {} }, [playMode, queue])

  const getNextTrack = () => {
    if (!queue || queue.length < 2) return null
    let nextIndex
    if (cachedNextIndex !== null) {
      debug('getNextTrack: using cachedNextIndex ->', cachedNextIndex)
      nextIndex = cachedNextIndex
    } else {
      // try the shared cache (maybe another component pre-calculated)
      try {
        const v = getCachedNext(queue)
        if (v !== null) {
          nextIndex = v
          setCachedNextIndex(nextIndex)
          debug('getNextTrack: found shared cachedNext ->', nextIndex)
        }
      } catch (e) { /* ignore */ }
      if (nextIndex == null) {
      if (playMode === 'shuffle' || playMode === 'randomNoRepeat') {
        ...
        debug('getNextTrack: pre-calculated authoritative nextIndex ->', nextIndex)
        setCachedNextIndex(nextIndex)
  ...
  // Keep Up Next preview in sync as index/mode/queue change
  useEffect(() => {
    if (queue && queue.length > 1) getNextTrack()
  ...
  useEffect(() => {
    ...
    debug(`🔧 Registered global audio elements for instance ${componentId.current}`);
  }, []);
  ...
  // Play-mode helpers handled by centralized randomizer util
  ...
  // Boot
  // Initial mount: set up queue from state or localStorage
  useEffect(() => {
    let initialQueue = [];
    let initialIndex = 0;
    ...
    if (djSingleLoad) {
      debug('DJ single track load detected:', djSingleLoad, 'NoAutoplay:', noAutoplay);
    ...
    window.ngksAudioControl = {
      stopAll: () => {
        debug('Global stop command received');
        ...
        setIsPlaying(false);
        setPosition(0);
        debug('Both audio elements stopped and reset');
      }
    };
  }, []);

  // Handle autoplay data after navigation (when component is already mounted)
  const autoplayProcessed = useRef(false);
  useEffect(() => {
    if (autoplayProcessed.current) return; // Only process autoplay once
    
    // Check for autoplay queue from sessionStorage (from navigation)
    const autoplayQueue = sessionStorage.getItem('ngks_autoplay_queue');
    if (autoplayQueue) {
      try {
        const parsedQueue = JSON.parse(autoplayQueue);
        debug('Autoplay queue detected after navigation:', parsedQueue);
        sessionStorage.removeItem('ngks_autoplay_queue');
        if (Array.isArray(parsedQueue) && parsedQueue.length > 0) {
          autoplayProcessed.current = true;
          setQueue(parsedQueue);
          // honor optional start index saved by the caller
          const idxRaw = sessionStorage.getItem('ngks_autoplay_index');
          if (idxRaw != null) {
            const idx = Number(idxRaw);
            setCurrentIndex(Number.isFinite(idx) ? clamp(idx, 0, parsedQueue.length - 1) : 0);
            sessionStorage.removeItem('ngks_autoplay_index');
          } else {
            setCurrentIndex(0);
          }
        }
      } catch (err) {
        console.error('Error parsing autoplay queue:', err);
      }
    }
    // Check for single autoplay file from sessionStorage (from navigation)
    else {
      const autoplayFile = sessionStorage.getItem('ngks_autoplay');
      if (autoplayFile) {
        debug('Autoplay file detected after navigation:', autoplayFile);
        sessionStorage.removeItem('ngks_autoplay');
        autoplayProcessed.current = true;
        setQueue([autoplayFile]);
        setCurrentIndex(0);
      }
    }
  }); // No dependency array - run on every render until autoplay is processed

  // Watch for playFile changes in location.state and update queue if needed
  useEffect(() => {
    if (location.state?.playFile) {
      debug('New song selected from library:', location.state.playFile);
  ...
  useEffect(() => {
    if (djMode && queue.length > 0 && queueA.length === 0 && queueB.length === 0) {
      ...
      debug('Auto-populated deck queues with', queue.length, 'tracks')
    }
  ...
  useEffect(() => {
    ...
    debug('Queue changed, fetching labels for:', queue);
    let cancelled = false
    ;(async () => {
      const entries = await Promise.all(queue.map(async p => {
        try { 
          debug('Calling getTrackByPath for label:', p);
  ...
  useEffect(() => {
    ...
    const noAutoplay = sessionStorage.getItem('ngks_no_autoplay') === 'true';
    if (noAutoplay) {
      debug('No-autoplay flag set - do not auto-start playback');
    ...
    if ((djMode || storedDjMode) && queue.length && queue[currentIndex] && !startInProgress.current) {
      debug('DJ Mode - queue loaded but no auto-start');
    ...
    if (queue.length && queue[currentIndex] && !startInProgress.current && !djMode && !storedDjMode) {
      debug('Starting playback - queue:', queue.length, 'currentIndex:', currentIndex, 'file:', queue[currentIndex]);
      ...
      debug('Start blocked - already in progress');
    } else if (!queue.length) {
      debug('No queue available');
    } else if (!queue[currentIndex]) {
      debug('No file at current index:', currentIndex);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [queue, currentIndex, djMode])

  useEffect(() => () => {
    debug(`NowPlaying component unmounting with ID: ${componentId.current}`);
    if (globalNowPlayingInstance === componentId.current) {
      debug(`Resetting global instance (was: ${globalNowPlayingInstance})`);
      globalNowPlayingInstance = null;
    }
    ...
    if (window.ngksAudioControl) {
      debug('Cleaning up global audio control');
  ...
  async function start(idx) {
    ...
    debug(`Begin start() for index ${idx}`);
    ...
    try {
      ...
      debug(`Stopping all audio elements`);
      ...
      debug(`Audio elements cleaned, reusing existing graph`);
  ...
  async function next() {
    ...
    try {
      ...
      let nextIdx
      if (cachedNextIndex !== null) {
        debug('next(): using cachedNextIndex ->', cachedNextIndex)
      ...
      if (window.api && window.api.createBlobUrl) {
        const blobResult = await window.api.createBlobUrl(nextPath);
        if (blobResult.ok) {
          nxt.src = blobResult.url;
        } else {
          console.warn('Failed to create blob URL for next track, falling back to file URL:', blobResult.error);
  ...
  const removeAt = (idx) => {
    ...
    if (!queue.length) return
    if (idx === currentIndex) {
      // Use non-destructive peek here as we're only choosing what will
      // be the next track after removal; do not mutate random bags.
  ...
  const loadTrackToDeckA = async (trackPath, autoPlay = true, skipConfirmation = false) => {
    try {
      ...
      // Load the track
      if (window.api && window.api.createBlobUrl) {
        const blobResult = await window.api.createBlobUrl(trackPath);
        if (blobResult.ok) {
          a1.current.src = blobResult.url;
        } else {
          console.warn('Failed to create blob URL, falling back to file URL:', blobResult.error);
    ...
    try {
      ...
      if (window.api && window.api.createBlobUrl) {
        ...
        if (blobResult.ok) {
          ...
          console.warn('Failed to create blob URL for Deck B, falling back to file URL:', blobResult.error);
          audioUrl = toLocal(trackPath);
        }
      } else {
        audioUrl = toLocal(trackPath);
      }
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                <button 
                  className={`btn btn-xs ${playMode === 'inOrder' ? 'btn-primary' : 'btn-ghost'}`}
                  onClick={() => setPlayMode('inOrder')}
                  title="In Order (Ctrl+1)"
                >
                  🔄
                </button>
                <button 
                  className={`btn btn-xs ${playMode === 'repeatAll' ? 'btn-primary' : 'btn-ghost'}`}
                  onClick={() => setPlayMode('repeatAll')}
                  title="Repeat All (Ctrl+2)"
                >
                  🔁
                </button>
                <button 
                  className={`btn btn-xs ${playMode === 'shuffle' ? 'btn-primary' : 'btn-ghost'}`}
                  onClick={() => setPlayMode('shuffle')}
                  title="Shuffle (Ctrl+4)"
                >
                  🔀
                </button>
                <button 
                  className={`btn btn-xs ${playMode === 'randomNoRepeat' ? 'btn-primary' : 'btn-ghost'}`}
                  onClick={() => setPlayMode('randomNoRepeat')}
                  title="Random (No Repeat) (Ctrl+5)"
                >
                  🎲
                </button>
                {/* Repeat One and True Random modes removed to reduce UI clutter */}
              </div>
            </div>
          </div>
        </div>
      ...
      {djMode ? (
        ...
        <>
          <div className="card space-y-3">
            ...
            {Array.isArray(trackMeta?.energyTrajectory) && trackMeta.energyTrajectory.length > 0 ? (
              <div className="mt-2">
                <EnergyWaveform
                  trajectory={trackMeta.energyTrajectory}
                  phrases={trackMeta.phraseData || []}
                  duration={duration || trackMeta?.duration}
                  height={40}
                  onSeek={(t) => { const A = getActive(); if (A) { A.currentTime = t; setPosition(t); } }}
                />
              </div>
            ) : null}
            ...
            <div className="text-xs opacity-60 flex items-center gap-2">
              ...
              <span 
                ...
                title="Play modes: Ctrl+1 (In Order), Ctrl+2 (Repeat All), Ctrl+4 (Shuffle), Ctrl+5 (Random - no repeat)"
              >
...
// Ensure file ends with a top-level close if any earlier edit removed it
}
