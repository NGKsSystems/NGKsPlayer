...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // Rock-tuned for AC/DC: High threshold + low-freq emphasis to ignore guitar riffs
      beatDetectionRef.current.adaptiveThreshold = Math.max(avgBass * 1.30, 80)  // 1.30x - filters guitar transients
      ...
      // Trigger beat pulse ONLY on strong low-frequency transients (kick drums)
      const now = Date.now()
      const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
      
      // Low-freq gating: Require strong bass + weak mids (drum kick signature)
      const isKick = bassAverage > beatDetectionRef.current.adaptiveThreshold && 
                     bassAverage > 80 &&  // Absolute minimum for real kick
                     bassAverage > midAverage * 0.8  // Bass must dominate (not guitar)
      
      if (beatPulseEnabledRef.current && isKick && timeSinceLastBeat > 200) {  // 200ms gate (~40% of 491ms beat)
