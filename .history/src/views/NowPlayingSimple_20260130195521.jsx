  ...
  const renameFile = async (track) => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        // Force immediate refresh by clearing tracks first, then reloading
        setTracks([])
        await loadTracks()
        
        // Force a re-render by updating search query (triggers filteredTracks recalculation)
        const currentQuery = searchQuery
        setSearchQuery('')
        setTimeout(() => setSearchQuery(currentQuery), 50)
        
        console.log('[Frontend] Library refreshed after rename')
      } else {
