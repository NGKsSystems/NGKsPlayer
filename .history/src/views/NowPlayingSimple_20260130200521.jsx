...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const handleLibraryRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.error('[Frontend] Library refresh failed:', err)
      showToast('❌ Library refresh failed', 'error')
    }
  ...
  const renameFile = async (track) => {
    ...
    try {
      ...
      if (result && result.success) {
        showToast('✅ File renamed successfully! Click "🔄 Refresh" button to update library.', 'success')
        console.log('[Frontend] File renamed successfully')
      } else {
