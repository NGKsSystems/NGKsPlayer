          ...
          if (typeof Meyda !== 'undefined') {
            meydaAnalyzerRef.current = Meyda.createMeydaAnalyzer({
              ...
              callback: (features) => {
                ...
                if (beatDetectionRef.current.fluxHistory.length > 40) {
                ...
                const fluxThreshold = avgFlux * 2.5  // Onset when flux is 2.5x average
                
                const now = Date.now()
                const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
                
                // Trigger on onset (spectral flux spike + high energy)
                if (flux > fluxThreshold && energy > 0.01 && timeSinceLastBeat > 150) {
                  beatDetectionRef.current.lastBeat = now
                  beatDetectionRef.current.beatStrength = Math.min(flux / avgFlux, 3)
                  
                  setBeatPulse(true)
                  setTimeout(() => setBeatPulse(false), 120)
                  
                  // Super peaks on massive onsets
                  if (flux > avgFlux * 4 && timeSinceLastBeat > 1000) {
                    beatDetectionRef.current.lastPeak = now
                    setPeakRotation(true)
                    setTimeout(() => setPeakRotation(false), 1200)
                  }
                }
              }
            })
          ...
          {currentTrack ? (
            <div>
              ...
              {detectedBPM && (
                ...
                >
                  <span className="text-sm font-bold glow-text" style={{ color: 'var(--accent-primary)' }}>
                    ♩ {detectedBPM} BPM
                  </span>
                </div>
              )}
