...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const renameFile = async (track) => {
    const currentName = track.filePath.split(/[\\\/]/).pop() // Get filename from path
    const newName = prompt('Enter new filename (with extension):', currentName)
    
    console.log('Rename - Current name:', currentName)
    console.log('Rename - New name:', newName)
    console.log('Rename - Full path:', track.filePath)
    ...
    try {
      console.log('Calling library:renameFile with:', { oldPath: track.filePath, newName })
