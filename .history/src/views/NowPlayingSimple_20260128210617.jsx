  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      if (waveformType === 'line') {
        ...
        const now = Date.now()
        if (beatPulseEnabled && bassAverage > beatDetectionRef.current.threshold && 
      ...
      <div className="flex-1 p-6 bg-gray-800 border-b border-gray-700 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 text-center" style={{ textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>
          {currentTrack ? (
            <div>
              <h2 
                className="text-5xl font-black mb-4 transition-all duration-100"
                style={{ 
                  fontFamily: '"Arial Black", "Impact", sans-serif',
                  transform: beatPulse ? 'scale(1.08)' : 'scale(1)',
                  textShadow: beatPulse 
                    ? '0 0 30px rgba(239, 68, 68, 0.9), 0 0 60px rgba(239, 68, 68, 0.5), 0 4px 12px rgba(0,0,0,1), 0 6px 20px rgba(0,0,0,0.8)' 
                    : '0 4px 12px rgba(0,0,0,1), 0 6px 20px rgba(0,0,0,0.8)',
                  WebkitTextStroke: '3px rgba(0, 0, 0, 1)',
                  paintOrder: 'stroke fill',
                  filter: beatPulse ? 'brightness(1.2) drop-shadow(0 0 10px rgba(255,255,255,0.5))' : 'brightness(1)',
                ...
                <div className="mt-8 pt-6 border-t border-gray-600">
                  ...
                  <p 
                    ...
                    style={{
                      ...
                      WebkitTextStroke: '2px rgba(0, 0, 0, 1)',
                      paintOrder: 'stroke fill',
                      textShadow: '0 2px 8px rgba(0,0,0,1), 0 4px 12px rgba(0,0,0,0.8)'
                    }}
                  >
                    {nextTrackInfo.title}
                  </p>
                  {nextTrackInfo.album && (
                    <p 
                      ...
                      style={{
                        ...
                        paintOrder: 'stroke fill',
                        textShadow: '0 2px 6px rgba(0,0,0,1), 0 0 15px rgba(14, 165, 233, 0.4)'
                      }}
                    >
                      {nextTrackInfo.album}
                    </p>
        ...
        <div className="absolute bottom-4 right-4 z-20 flex gap-2">
          ...
          <button
            className={`px-3 py-1 rounded text-xs ${beatPulseEnabled ? 'bg-purple-600' : 'bg-gray-700 hover:bg-gray-600'}`}
            onClick={() => setBeatPulseEnabled(!beatPulseEnabled)}
            title="Toggle Beat Pulse"
          >
            {beatPulseEnabled ? '💓 Pulse' : '🔇 Pulse'}
          </button>
        </div>
      </div>
