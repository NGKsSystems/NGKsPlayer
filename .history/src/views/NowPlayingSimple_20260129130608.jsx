    ...
    const setupWaveform = async () => {
      ...
      try {
        ...
        if (!audioContextRef.current) {
          ...
          // Initialize Meyda for professional beat detection
          if (typeof Meyda !== 'undefined') {
            meydaAnalyzerRef.current = Meyda.createMeydaAnalyzer({
              audioContext: audioContextRef.current,
              source: source,
              bufferSize: 512,
              featureExtractors: ['rms', 'spectralFlux', 'energy'],
              callback: (features) => {
                if (!features) return
                
                // Use spectral flux for onset detection (better than just bass)
                const flux = features.spectralFlux || 0
                const energy = features.energy || 0
                const rms = features.rms || 0
                
                // Track flux history for adaptive threshold
                if (!beatDetectionRef.current.fluxHistory) {
                  beatDetectionRef.current.fluxHistory = []
                }
                beatDetectionRef.current.fluxHistory.push(flux)
                if (beatDetectionRef.current.fluxHistory.length > 20) {  // Shorter history for faster adaptation
                  beatDetectionRef.current.fluxHistory.shift()
                }
                
                const avgFlux = beatDetectionRef.current.fluxHistory.reduce((a, b) => a + b, 0) / beatDetectionRef.current.fluxHistory.length
                const fluxThreshold = Math.max(avgFlux * 1.2, 0.0005)  // Very sensitive (was 1.5, originally 2.5)
                
                const now = Date.now()
                const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
                
                // Hyper-aggressive triggering - catches kicks, snares, hihats, cymbals
                const isOnset = (flux > fluxThreshold || rms > 0.02 || energy > 0.008) && timeSinceLastBeat > 80  // Multiple triggers, fast gate
                
                if (isOnset && waveformType !== 'none' && beatPulseEnabledRef.current) {
                  console.log('🔊 BEAT DETECTED!', { flux: flux.toFixed(4), rms: rms.toFixed(4), energy: energy.toFixed(4), threshold: fluxThreshold.toFixed(4) })
                  beatDetectionRef.current.lastBeat = now
                  beatDetectionRef.current.beatStrength = Math.min(flux / (avgFlux + 0.001), 3)
                  
                  setBeatPulse(true)
                  setTimeout(() => setBeatPulse(false), 100)  // Shorter pulse duration
                  
                  // Super peaks on massive onsets
                  if (flux > avgFlux * 3 && timeSinceLastBeat > 800) {  // Lowered requirements
