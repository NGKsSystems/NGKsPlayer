...
export default function TagEditor({ onNavigate }){
  ...
  async function analyze(){
    ...
    try{
      // Get track data from database to get the track ID
      const currentTags = await window.api.getTags(cleanPath)
      
      // Get the track ID by querying via getTrackByPath
      const trackData = await window.api.getTrackByPath(cleanPath)
      
      if (!trackData) {
        throw new Error('Track not found in database. Please add it to your library first.')
      }
      
      setStatus('Analyzing audio (this may take 10-30 seconds)...')
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4">
      ...
      {activeTab === 'single' ? (
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input 
              className="flex-1 px-2 py-1.5 text-sm bg-gray-700 border border-gray-600 rounded text-white placeholder-gray-400" 
            ...
            />
            <button
              className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-sm font-medium whitespace-nowrap"
              onClick={async () => {
                try {
                  const result = await window.api.invoke('select-file', {
                    filters: [
