...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // HYPER AGGRESSIVE for rock - catch every kick/snare/hihat
      beatDetectionRef.current.adaptiveThreshold = Math.max(avgBass * 1.075, 38)  // Fine-tuned for AC/DC
      ...
      // HYPER SENSITIVE - catches almost everything
      if (beatPulseEnabledRef.current && 
          (bassAverage > beatDetectionRef.current.adaptiveThreshold || midAverage > avgMid * 1.15) &&  // Bass OR mids spike
          timeSinceLastBeat > 80) {  // Very fast gate for rapid hits
