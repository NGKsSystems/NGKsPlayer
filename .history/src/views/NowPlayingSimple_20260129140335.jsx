...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // Rock-tuned sweet spot: 1.23x threshold + high minimum
      beatDetectionRef.current.adaptiveThreshold = Math.max(avgBass * 1.23, 78)  // Balanced for kicks
      ...
      // Simple but effective: Strong low-freq transients only
      const now = Date.now()
      const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
      
      // Detect kick: bass spike + significant absolute level (no mid comparison - too strict)
      const isKick = bassAverage > beatDetectionRef.current.adaptiveThreshold && bassAverage > 78
      
      if (beatPulseEnabledRef.current && isKick && timeSinceLastBeat > 220) {  // 220ms gate (~half beat)
