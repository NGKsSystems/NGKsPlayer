...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // ROCK MODE: Lower threshold + weighted low-freq for kick emphasis
      const weightedBass = bassAverage * 1.5  // Amplify kick energy vs guitar
      const baseThreshold = avgBass * 1.20  // Sweet spot for rock (was 1.23)
      beatDetectionRef.current.adaptiveThreshold = Math.max(baseThreshold, 85)  // Higher min (was 78)
      ...
      // Kick detection: weighted bass spike + absolute minimum
      const now = Date.now()
      const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
      
      // Use weighted bass for detection (emphasizes low-freq transients)
      const isKick = weightedBass > beatDetectionRef.current.adaptiveThreshold * 1.5 && bassAverage > 85
      
      if (beatPulseEnabledRef.current && isKick && timeSinceLastBeat > 230) {  // 230ms gate
