...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const beatDetectionRef = useRef({ lastBeat: 0, threshold: 165, bassHistory: [], lastPeak: 0 })
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // Beat detection - runs for all visual modes except 'none'
      // Analyze bass frequencies (first ~10% of spectrum)
      ...
      // Track bass history for peak detection
      ...
      if (beatDetectionRef.current.bassHistory.length > 30) {
      ...
      // Trigger beat pulse if enabled, bass is strong, enough time has passed, and visualization is active
      const now = Date.now()
      if (waveformType !== 'none' && beatPulseEnabledRef.current && bassAverage > beatDetectionRef.current.threshold && 
          now - beatDetectionRef.current.lastBeat > 200) {
        beatDetectionRef.current.lastBeat = now
        console.log('🎵 BEAT! Bass:', bassAverage.toFixed(2), '| Mode:', waveformType)
        setBeatPulse(true)
        setTimeout(() => setBeatPulse(false), 150)
        
        // Detect PEAK beats (when bass is significantly higher than average)
        // Peak = bass is 50% higher than recent average
        if (bassAverage > avgBass * 1.5 && now - (beatDetectionRef.current.lastPeak || 0) > 3000) {
          beatDetectionRef.current.lastPeak = now
          console.log('🌟 PEAK DETECTED! Orbital rotation triggered!')
          setPeakRotation(true)
          // 3 rotations at 0.5s each = 1.5 seconds total
  ...
  return (
    <>
              ...
              {currentTrack.album && (
                <p 
                  ...
                  style={{
                    ...
                    color: '#0ea5e9', // bright cyan-blue - contrasts perfectly with yellow
                    WebkitTextStroke: '2px rgba(14, 165, 233, 0.4)',
                    paintOrder: 'stroke fill',
                    textShadow: '0 3px 10px rgba(0,0,0,1), 0 5px 15px rgba(0,0,0,0.8), 0 0 20px rgba(14, 165, 233, 0.5)',
                    letterSpacing: '0.02em'
                  }}
                >
                  {currentTrack.album}
                  ...
                  {nextTrackInfo.album && (
                    <p 
                      ...
                      style={{
                        ...
                        color: '#0ea5e9',
                        WebkitTextStroke: '2.5px rgba(14, 165, 233, 0.4)',
                        paintOrder: 'stroke fill',
                        textShadow: '0 3px 10px rgba(0,0,0,1), 0 5px 15px rgba(0,0,0,1), 0 0 20px rgba(14, 165, 233, 0.5)',
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Search Bar - Sticky */}
        <div className="sticky top-0 z-10 bg-gray-900 p-6 pb-4 border-b border-gray-700">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button
              ...
              className="px-3 py-1 bg-gray-700 hover:bg-gray-600 border border-gray-600 rounded text-sm flex items-center gap-1"
