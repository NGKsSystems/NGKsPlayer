...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  */
  ...
  const renameFile = async (track) => {
    ...
    try {
      // Ensure we have the original file extension for backup
      const originalExt = track.filePath ? track.filePath.split('.').pop() : 'mp3'
      let finalNewName = newFileName.trim()
      
      // Frontend safety: add extension if missing (backup for backend logic)
      if (!finalNewName.includes('.') && originalExt) {
      ...
      console.log('[Frontend] Track object:', track)
      ...
      if (result && result.success) {
        ...
        await loadTracks()
        console.log('[Frontend] Tracks reloaded after rename')
      } else {
