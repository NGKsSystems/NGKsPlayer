...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // Beat detection - runs for all waveform types
      ...
      // Trigger beat pulse if enabled, bass is strong, and enough time has passed
      const now = Date.now()
      if (beatPulseEnabledRef.current && bassAverage > beatDetectionRef.current.threshold && 
          now - beatDetectionRef.current.lastBeat > 200) {
        beatDetectionRef.current.lastBeat = now
        console.log('🎵 BEAT! Bass:', bassAverage.toFixed(2), '| Enabled:', beatPulseEnabledRef.current)
        setBeatPulse(true)
