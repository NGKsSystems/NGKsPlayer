...
import { pickNextIndex } from '../lib/randomizer'
...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  // Audio ref hook (manages audio element and event listeners)
  ...
  const playTrack = async (track, index) => {
    ...
    audioRef.current.src = `ngksplayer://${filePath}`
    ...
    try {
      ...
      if (index !== undefined && tracks.length > 0) {
        const nextIdx = pickNextIndex(index, tracks.length, playMode)
  ...
  function nextTrack() {
    ...
    if (cachedNextIndex !== null && cachedNextIndex !== undefined) {
      ...
      nextIndex = pickNextIndex(currentIndexRef.current ?? 0, tracks.length, playMode)
    }
  ...
  function prevTrack() {
    ...
    if (playMode === 'inOrder' || playMode === 'repeatAll') {
      ...
      prevIndex = pickNextIndex(currentIndexRef.current ?? 0, tracks.length, playMode)
    }
  ...
  const showInExplorer = (track) => {
    window.api.showInExplorer(track.path)
    setContextMenu(null)
  }
  ...
  const analyzeTrack = async (track) => {
    ...
    try {
      const result = await window.api.analyzeTrack(track.path)
      if (result.success) {
  ...
  const renameFile = async (track) => {
    ...
    try {
      const result = await window.api.renameTrack(track.path, newFileName)
      if (result.success) {
  ...
  const deleteFile = async (track) => {
    ...
    try {
      const result = await window.api.deleteTrack(track.path)
      if (result.success) {
