...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const beatDetectionRef = useRef({ lastBeat: 0, threshold: 200 })
      ...
      <div className="flex-1 p-6 bg-gray-800 border-b border-gray-700 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 text-center" style={{ textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>
          {currentTrack ? (
            <div>
              <h2 
                className="text-4xl font-bold mb-4 transition-transform duration-100"
                style={{ 
                  transform: beatPulse ? 'scale(1.05)' : 'scale(1)',
                  textShadow: beatPulse ? '0 0 20px rgba(239, 68, 68, 0.8)' : '0 2px 8px rgba(0,0,0,0.8)'
                }}
              >
              ...
              {nextTrackInfo && (
                <div className="mt-8 pt-6 border-t border-gray-600">
                  ...
                  <p className="text-lg font-medium">{nextTrackInfo.title || 'Unknown'}</p>
                  <p className="text-sm text-gray-400">{nextTrackInfo.artist || 'Unknown'}</p>
                </div>
              )}
            </div>
          ) : (
