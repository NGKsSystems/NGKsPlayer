...
export default function NowPlayingBasic({ onNavigate }) {
    ...
    const setupWaveform = async () => {
      ...
      try {
        ...
        if (!audioContextRef.current) {
          ...
          // FALLBACK: Use Web Audio API directly for beat detection (Meyda had bugs)
          // Simple but effective onset detection using frequency analysis
          const detectBeats = () => {
            if (!analyzerRef.current || !beatPulseEnabledRef.current) return  // Removed waveformType check - it was stale
            
            const bufferLength = analyzerRef.current.frequencyBinCount
            const dataArray = new Uint8Array(bufferLength)
            analyzerRef.current.getByteFrequencyData(dataArray)
            
            // Focus on bass/low-mid range for kick/snare detection (0-15% of spectrum)
            ...
            // Track history
            if (!beatDetectionRef.current.beatHistory) {
              beatDetectionRef.current.beatHistory = []
            }
            beatDetectionRef.current.beatHistory.push(bassAvg)
            if (beatDetectionRef.current.beatHistory.length > 15) {
              beatDetectionRef.current.beatHistory.shift()
            }
            
            const avgBass = beatDetectionRef.current.beatHistory.reduce((a, b) => a + b, 0) / beatDetectionRef.current.beatHistory.length
            const threshold = avgBass * 1.25  // 25% above average
            
            const now = Date.now()
            const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
            
            // Detect beat
            if (bassAvg > threshold && bassAvg > 30 && timeSinceLastBeat > 120) {  // Added minimum bass level
              console.log('🔊 BEAT DETECTED!', { bass: bassAvg.toFixed(1), avg: avgBass.toFixed(1), threshold: threshold.toFixed(1) })
              beatDetectionRef.current.lastBeat = now
              beatDetectionRef.current.beatStrength = Math.min(bassAvg / avgBass, 2.5)
              
              setBeatPulse(true)
              setTimeout(() => setBeatPulse(false), 140)
              
              // Peak detection
              if (bassAvg > avgBass * 1.8 && timeSinceLastBeat > 800) {
                beatDetectionRef.current.lastPeak = now
                setPeakRotation(true)
                setTimeout(() => setPeakRotation(false), 1000)
              }
            }
          }
          
          // Run beat detection in animation loop
          console.log('[BeatDetection] detectBeats function initialized')
          beatDetectionRef.current.detectBeats = detectBeats
        }
        ...
        console.log('[Waveform] Starting animation loop, isPlaying:', isPlaying, 'waveformType:', waveformType)
        drawWaveform()
      } catch (err) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // Run beat detection
      if (beatDetectionRef.current.detectBeats) {
        beatDetectionRef.current.detectBeats()
      } else {
        console.warn('[Waveform] detectBeats function not initialized!')
      }
      ...
      // Lower threshold for better detection (was 1.4, now 1.2)
      beatDetectionRef.current.adaptiveThreshold = Math.max(avgBass * 1.2, 80)
      ...
      // Trigger beat pulse on strong bass hits (lowered threshold for better detection)
      const now = Date.now()
      const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
      
      if (waveformType !== 'none' && beatPulseEnabledRef.current && 
          bassAverage > beatDetectionRef.current.adaptiveThreshold && 
          timeSinceLastBeat > 180) {  // Reduced from 200ms to 180ms
