...
import * as BeatDetector from 'web-audio-beat-detector'  // Professional beat detection
import { useEssentiaBeatDetection } from '../hooks/useEssentiaBeatDetection'  // Essentia.js beat detection

export default function NowPlayingBasic({ onNavigate }) {
  const { currentTheme, changeTheme, themes } = useTheme()
  const [showThemeMenu, setShowThemeMenu] = useState(false)
  const audioRef = useRef(new Audio())
  const beatDetectorRef = useRef(null)  // Real-time beat detector
  ...
  // CRITICAL: Refs for beat detection (updated from sliders)
  ...
  const [useEssentiaBeats, setUseEssentiaBeats] = useState(false) // Toggle between Essentia and custom detection
  ...
  // Essentia.js beat detection hook (professional-grade)
  ...
  const { isReady: essentiaReady } = useEssentiaBeatDetection({
    ...
    onBeat: ({ time, strength, confidence }) => {
      ...
      // Super peak detection for dramatic effects
    ...
    const setupWaveform = async () => {
      ...
      try {
        ...
        if (!audioContextRef.current) {
          ...
          analyzerRef.current.smoothingTimeConstant = 0.6  // Lower = more responsive (was default 0.8)
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // TEST: Verify draw loop is running
      if (Math.random() < 0.01) {
        console.log('✅ Draw loop running, isPlaying:', isPlaying, 'waveformType:', waveformType)
      }
      ...
      if (beatDetectionRef.current.bassHistory.length > beatHistoryLength) {
      ...
      // Calculate adaptive threshold (1.2x average bass - more sensitive)
      const avgBass = beatDetectionRef.current.bassHistory.reduce((a, b) => a + b, 0) / beatDetectionRef.current.bassHistory.length
      
      // USE REFS (not state) - these update in real-time from sliders
      const threshold = beatThresholdRef.current
      const minimum = beatMinRef.current
      const gate = beatGateRef.current
      ...
      if (beatDetectionRef.current.debugCounter === undefined) beatDetectionRef.current.debugCounter = 0
      ...
      if (!useEssentiaBeats && beatPulseEnabledRef.current && bassSpike && aboveMin && timeSinceLastBeat > gate) {
        console.log('🥁 KICK', { bass: bassAverage.toFixed(1), threshold: beatDetectionRef.current.adaptiveThreshold.toFixed(1), time: (now/1000).toFixed(2) })
        
        console.log('🔊 BEAT!', { bass: bassAverage.toFixed(1), threshold: beatDetectionRef.current.adaptiveThreshold.toFixed(1), enabled: beatPulseEnabledRef.current })
        ...
        if (bassAverage > avgBass * 1.6 && timeSinceLastBeat > 1500) {  // Lowered from 1.8 and 2000
      ...
      // Get actual computed theme colors (CSS vars don't work in canvas)
      ...
      // Apply glow effect on beat pulse
      if (beatPulse) {
        ctx.shadowBlur = 20
        ctx.shadowColor = getDynamicColor()
      } else {
        ctx.shadowBlur = 0
      }
      ...
      if (waveformType === 'line') {
        ...
        ctx.lineWidth = beatPulse ? 4 : 2  // Thicker line on beat
        ...
        for (let i = 0; i < bufferLength; i++) {
          ...
          const y = (v * height) / 2  // Accurate amplitude
        ...
        for (let i = 0; i < bufferLength; i++) {
          const barHeight = (frequencyData[i] / 255) * height * 0.85  // Accurate amplitude
        ...
        const pulseScale = beatPulse ? 1.4 : 1  // Strong visible pulse

        for (let i = 0; i < bufferLength; i++) {
          const angle = (i / bufferLength) * Math.PI * 2
          const amplitude = (dataArray[i] / 128.0 - 1) * radius * 0.6 * pulseScale  // Moderate amplitude
  ...
  const playTrack = async (track, index) => {
    try {
      ...
      setDetectedBPM(track.bpm || null)  // Use existing BPM or clear for detection
      ...
      audioRef.current.addEventListener('canplay', async () => {
        ...
        if (audioContextRef.current && !track.bpm) {
          try {
            // Fetch and decode audio for BPM analysis
            ...
            if (window.api?.updateTrackBPM) {
              window.api.updateTrackBPM(track.id, bpm)
            }
          } catch (err) {
            console.warn('[BPM] Detection failed:', err)
          }
        } else if (track.bpm) {
          setDetectedBPM(track.bpm)
        }
      }, { once: true })
      ...
      <div className="p-4 bg-gray-800 border-b border-gray-700 flex items-center justify-center relative overflow-hidden" style={{ height: '200px' }}>
        ...
        <div className="absolute bottom-4 left-4 z-30 flex gap-2">
          ...
          <button
            ...
            onClick={() => setShowBeatControls(!showBeatControls)}
        ...
        {/* Beat Detection Control Panel - Draggable */}
        {showBeatControls && (
          <div 
            className="fixed bg-gray-800 border border-gray-600 rounded-lg shadow-2xl w-80"
            style={{ 
              left: `${panelPos.x}px`, 
              top: `${panelPos.y}px`,
              zIndex: 9999,
          ...
          >
            ...
            >
              <h3 className="text-sm font-bold text-gray-200">⚙️ Beat Detection</h3>
              <button
                onClick={() => setShowBeatControls(false)}
            ...
            <div className="p-4 max-h-96 overflow-y-auto">
              ...
              <div className="mb-4 p-3 bg-gray-900 rounded border border-blue-500">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-bold text-blue-400">Essentia.js Beat Detection</h4>
                    <p className="text-xs text-gray-400 mt-1">Professional ML-based beat tracking</p>
                    {essentiaReady && <p className="text-xs text-green-400 mt-1">✓ WASM Loaded</p>}
                  </div>
                  <button
              ...
              {useEssentiaBeats && essentiaReady && (
                <div className="mb-4">
                  ...
                  >
                    <span className="text-sm font-bold text-blue-300">⚡ Advanced (Essentia ML)</span>
                    <span className="text-gray-400">{showAdvanced ? '▼' : '▶'}</span>
                  </button>
                  
                  {showAdvanced && (
                    <div className="mt-3 p-3 bg-gray-900 rounded border border-blue-500 space-y-3 text-xs">
                      {/* Onset Method */}
                      <div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full"
                        />
                        <p className="text-gray-500 text-xs mt-1">Lower = more pulses</p>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full"
                        />
                        <p className="text-gray-500 text-xs mt-1">Prevents false onsets in quiet sections</p>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full"
                        />
                        <p className="text-gray-500 text-xs mt-1">Higher = fewer false pulses</p>
                      </div>
                      
                      {/* Low-Freq Weight */}
                      <div>
                        <label className="flex justify-between text-gray-300 mb-1">
                          <span>Low-Freq Boost</span>
                          <span className="font-mono text-blue-400">{lowFreqWeight.toFixed(1)}×</span>
                        </label>
                        <input
                          type="range"
                          min="1.0"
                          ...
                          className="w-full"
                        />
                        <p className="text-gray-500 text-xs mt-1">More emphasis on kick drum</p>
                      </div>
                      
                      {/* Min Beat Interval */}
                      <div>
                        <label className="flex justify-between text-gray-300 mb-1">
                          <span>Minimum Beat Gap</span>
                        ...
                        <input
                          ...
                          className="w-full"
                        />
                        <p className="text-gray-500 text-xs mt-1">Prevents double-triggering</p>
                      </div>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="grid">Grid Lock</option>
                        </select>
                        <p className="text-gray-500 text-xs mt-1">Grid Lock snaps to steady beat grid</p>
                      </div>
              ...
              <div>
                <label className="flex justify-between text-gray-300 mb-1">
                  ...
                  <span className="font-mono text-purple-400">{beatSpikeThreshold.toFixed(2)}x</span>
                </label>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setBeatSpikeThreshold(Math.max(1.0, beatSpikeThreshold - 0.01))}
                  ...
                  />
                  <button
                    onClick={() => setBeatSpikeThreshold(Math.min(2.0, beatSpikeThreshold + 0.01))}
                    className="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-sm"
                  >▲</button>
                </div>
              ...
              <div>
                <label className="flex justify-between text-gray-300 mb-1">
                  ...
                  <span className="font-mono text-purple-400">{beatMinimum}</span>
                </label>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setBeatMinimum(Math.max(50, beatMinimum - 1))}
                    className="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-sm"
                  >▼</button>
                  <input
                    type="range"
                  ...
                  />
                  <button
                    onClick={() => setBeatMinimum(Math.min(200, beatMinimum + 1))}
                    className="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-sm"
                  >▲</button>
                </div>
              ...
              <div>
                <label className="flex justify-between text-gray-300 mb-1">
                  <span>Time Gate</span>
                  <span className="font-mono text-purple-400">{beatGate}ms</span>
                </label>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setBeatGate(Math.max(100, beatGate - 5))}
                    className="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-sm"
                  >▼</button>
                  <input
                    type="range"
                  ...
                  />
                  <button
                    onClick={() => setBeatGate(Math.min(500, beatGate + 5))}
                    className="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-sm"
                  >▲</button>
                </div>
              ...
              <div>
                <label className="flex justify-between text-gray-300 mb-1">
                  ...
                  <span className="font-mono text-purple-400">{beatHistoryLength}</span>
                </label>
                <div className="flex items-center gap-2">
                  <button
              ...
              <button
                ...
                className="w-full mt-2 px-3 py-2 bg-red-600 hover:bg-red-700 rounded text-white text-sm"
              >
                Reset to Defaults
              </button>
              </div>
