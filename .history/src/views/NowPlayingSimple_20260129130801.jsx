        ...
        if (!audioContextRef.current) {
          ...
          if (typeof Meyda !== 'undefined') {
            meydaAnalyzerRef.current = Meyda.createMeydaAnalyzer({
              ...
              source: source,
              bufferSize: 512,
              featureExtractors: ['rms', 'energy', 'zcr'],  // Removed broken spectralFlux, using zero-crossing rate
              callback: (features) => {
                if (!features) return
                
                // Use RMS and energy for onset detection (spectralFlux was crashing)
                const rms = features.rms || 0
                const energy = features.energy || 0
                const zcr = features.zcr || 0  // Zero-crossing rate for high-freq transients
                
                // Track RMS history for adaptive threshold
                if (!beatDetectionRef.current.rmsHistory) {
                  beatDetectionRef.current.rmsHistory = []
                }
                beatDetectionRef.current.rmsHistory.push(rms)
                if (beatDetectionRef.current.rmsHistory.length > 20) {
                  beatDetectionRef.current.rmsHistory.shift()
                }
                
                const avgRms = beatDetectionRef.current.rmsHistory.reduce((a, b) => a + b, 0) / beatDetectionRef.current.rmsHistory.length
                const rmsThreshold = Math.max(avgRms * 1.3, 0.02)  // 1.3x average RMS
                
                const now = Date.now()
                const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
                
                // Onset detection: RMS spike OR high energy OR high zero-crossing (cymbals/hihats)
                const isOnset = (rms > rmsThreshold || energy > 0.015 || zcr > 0.08) && timeSinceLastBeat > 100
                
                if (isOnset && waveformType !== 'none' && beatPulseEnabledRef.current) {
                  console.log('🔊 BEAT DETECTED!', { rms: rms.toFixed(4), energy: energy.toFixed(4), zcr: zcr.toFixed(4), threshold: rmsThreshold.toFixed(4) })
                  beatDetectionRef.current.lastBeat = now
                  beatDetectionRef.current.beatStrength = Math.min(rms / (avgRms + 0.001), 3)
                  
                  setBeatPulse(true)
                  setTimeout(() => setBeatPulse(false), 120)
                  
                  // Super peaks on massive onsets
                  if (rms > avgRms * 2.5 && timeSinceLastBeat > 800) {
