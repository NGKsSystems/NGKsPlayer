  ...
  const renameFile = async (track) => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        console.log('[Frontend] Calling loadTracks to refresh...')
        
        // Force immediate refresh with multiple strategies
        try {
          // Strategy 1: Clear and reload tracks
          setTracks([])
          
          // Strategy 2: Force fresh data load
          const freshTracks = await window.api.invoke('library:getTracks', {})
          if (freshTracks && Array.isArray(freshTracks)) {
            setTracks(freshTracks)
            console.log('[Frontend] Fresh tracks loaded:', freshTracks.length)
          }
          
          // Strategy 3: Reset search to trigger filteredTracks recalc
          const originalQuery = searchQuery
          setSearchQuery(`__refresh_${Date.now()}`)
          setTimeout(() => {
            setSearchQuery(originalQuery)
            console.log('[Frontend] Search reset complete')
          }, 100)
          
        } catch (refreshErr) {
          console.error('[Frontend] Refresh failed, falling back to loadTracks:', refreshErr)
          await loadTracks()
        }
        
        console.log('[Frontend] Library refresh strategies completed')
      } else {
