...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // Focus on drum transients - higher threshold to ignore guitars
      beatDetectionRef.current.adaptiveThreshold = Math.max(avgBass * 1.18, 60)  // Higher - only real kicks/snares
      ...
      // Only trigger on strong bass transients (drums), ignore sustained guitar
      if (beatPulseEnabledRef.current && 
          bassAverage > beatDetectionRef.current.adaptiveThreshold &&  // Removed mid detection - was catching guitars
          timeSinceLastBeat > 120) {  // Slower gate for cleaner beat timing
