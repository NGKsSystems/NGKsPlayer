...
export default function NowPlayingBasic({ onNavigate }) {
  ...
  const drawWaveform = () => {
    ...
    const draw = () => {
      ...
      // SIMPLE STRONG DETECTION - just look for ANY strong bass hit
      const strongKick = bassAverage > 120  // Absolute threshold - kick drums are LOUD
      
      const now = Date.now()
      const timeSinceLastBeat = now - beatDetectionRef.current.lastBeat
      
      if (beatPulseEnabledRef.current && strongKick && timeSinceLastBeat > 200) {
