 ...
 * NGKsPlayer
 *
 * Module: audioProcessing.test.js
...
// Mock only the global AudioContext to prevent hanging
...
describe('Audio Processing Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Create mock audio buffer with real Float32Array data
    const testData = new Float32Array(44100);
    // Generate sine wave test data
    for (let i = 0; i < 44100; i++) {
      testData[i] = Math.sin(2 * Math.PI * 440 * i / 44100) * 0.5; // 440Hz sine wave
    }
    ...
    mockAudioBuffer = {
      ...
      sampleRate: 44100,
      getChannelData: jest.fn((channel) => testData) // Return the same test data for both channels
    };
  });
  ...
  describe('parseAudioMetadata', () => {
    ...
    test('should detect AIFF format', () => {
      ...
      // Write FORM header (AIFF signature)
      const form = 'FORM';
      for (let i = 0; i < form.length; i++) {
        view.setUint8(i, form.charCodeAt(i));
      }
      
      // Write AIFF type at offset 8
      const aiff = 'AIFF';
      for (let i = 0; i < aiff.length; i++) {
        view.setUint8(8 + i, aiff.charCodeAt(i));
      }
...
describe('Audio Processing Integration Tests', () => {
  test('should handle complete extract-to-export workflow', async () => {
    // Create smaller test audio buffer for faster testing
    const sourceBuffer = {
      numberOfChannels: 2,
      length: 2048, // Much smaller - just 2048 samples instead of 88200
      sampleRate: 44100,
    ...
    // Extract a clip (smaller portion)
    const clippedBuffer = extractAudioClip(sourceBuffer, 0.0, 0.01); // 0.01 second clip
    expect(clippedBuffer.length).toBeLessThanOrEqual(1024); // Much smaller
