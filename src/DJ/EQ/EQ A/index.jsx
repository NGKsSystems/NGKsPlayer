...
const EQA = ({ id, deck = 'A', audioContext, gainNode, pannerNode, onStyleChange = () => {}, style = {}, minimized = false, onMinimize = () => {}, ...props }) => {
      ...
      eqFilters.forEach((filter, index) => {
        if (filter && filter.gain) {
          try {
            ...
            console.error(`[EQA] Failed to update filter ${index}:`, error);
          }
        }
      });
    }
      ...
      eqFilters.forEach((filter, index) => {
        if (filter && filter.gain) {
          try {
            ...
            console.error(`[EQA] Failed to apply preset to band ${index}:`, error);
          }
        }
      });
    }
      ...
      eqFilters.forEach((filter, index) => {
        if (filter && filter.gain) {
          try {
            ...
            console.error(`[EQA] Failed to reset band ${index}:`, error);
          }
        }
      });
    }
  ...
  return (
    <div 
      ref={widgetRef}
      className={`eq-${deck.toLowerCase()}-widget ${isDragging ? 'dragging' : ''} ${minimized ? 'collapsed' : ''}`} 
      style={currentStyle} 
      onMouseDown={handleMouseDown}
      {...props}
    >
      <div className={`eq-${deck.toLowerCase()}-header`} onMouseDown={handleMouseDown}>
        <h3>EQ {deck} {minimized && <span style={{fontSize: '10px', opacity: 0.7}}>(collapsed)</span>}</h3>
