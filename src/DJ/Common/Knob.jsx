...
const Knob = ({ 
  ...
  const getLabel = () => {
    const typeLabel = controlType ? controlType.charAt(0).toUpperCase() + controlType.slice(1) : '';
    return typeLabel && channel ? `${typeLabel} ${channel}` : (label || '');
  };
  ...
  return (
    <div className="knob-container" data-control-type={controlType}>
      ...
      >
        ...
        ></div>
        <div className="knob-label-center">
          {getLabel()}
        </div>
      </div>
