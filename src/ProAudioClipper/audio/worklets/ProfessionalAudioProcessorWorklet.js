 ...
 * NGKsPlayer
 *
 * Module: ProfessionalAudioProcessorWorklet.js
...
class ProfessionalAudioProcessorWorklet extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    // Use a simple log for worklet context (console.log should work)
  ...
  process(inputs, outputs, parameters) {
    // Use currentTime for performance monitoring (performance.now() not available in worklet)
    ...
    for (let i = 0; i < frameLength; i++) {
      ...
      if (parameters.eqBypass[0] < 0.5) {
        [leftSample, rightSample] = this.eq.process(leftSample, rightSample);
      }
      
      // 2. Compressor
      if (parameters.compBypass[0] < 0.5) {
        [leftSample, rightSample] = this.compressor.process(leftSample, rightSample);
      }
      
      // 3. Reverb (last in chain)
      if (parameters.reverbBypass[0] < 0.5) {
        [leftSample, rightSample] = this.reverb.process(leftSample, rightSample);
      }
    ...
    // Performance monitoring using simple frame counting
    const endTime = currentTime;
    const processingTime = (endTime - startTime) * 1000; // Convert to milliseconds estimate
  ...
  sendMeteringData() {
    ...
    this.port.postMessage({
      ...
      data: {
        ...
        compressorGR: this.compressor.gainReduction || 0
      }
    });
  }
  ...
  sendPerformanceStats() {
    ...
    // Simplified CPU usage estimation (frame processing time vs available time)
    const frameTime = 128 / sampleRate * 1000; // 128 samples per frame in milliseconds
    const cpuUsage = Math.min(100, (avgProcessingTime / frameTime) * 100);
