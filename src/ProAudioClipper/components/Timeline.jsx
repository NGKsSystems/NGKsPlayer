...
const Timeline = forwardRef(({
  ...
  // Separate handler for ruler clicks - seek directly
  const handleRulerClick = useCallback((e) => {
    const rect = e.currentTarget.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const time = pixelToTime(x);
    
    // Call seek directly for ruler clicks
    if (onSeek) {
      onSeek(time);
    }
    
    // Prevent event from bubbling to prevent timeline selection
    e.stopPropagation();
  }, [pixelToTime, onSeek]);
  ...
  const handleWheel = useCallback((e) => {
    ...
    if (e.ctrlKey) {
      ...
      // Horizontal scroll - improved sensitivity
      const scrollSensitivity = duration / 50; // Scroll amount based on duration
      const scrollAmount = (e.deltaY / 100) * scrollSensitivity;
      if (onViewportChange) {
        onViewportChange(viewportStart + scrollAmount);
      }
    }
  }, [pixelToTime, zoomLevel, viewportStart, duration, onViewportChange]);
  ...
  // Add wheel event listeners with proper passive handling
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
