...
class AIFFCodec {
  ...
  static generateAIFF(audioBuffer, options = {}) {
    ...
    // FORM header (8) + AIFF type (4) + COMM chunk (8 + 18) + SSND chunk (8 + 8 + data)
    const commonChunkSize = 18; // COMM chunk is always 18 bytes
    const totalFileSize = 8 + 4 + 8 + commonChunkSize + 8 + 8 + totalDataBytes;
    ...
    view.setUint32(offset, totalFileSize - 8, false); offset += 4; // File size minus FORM header
...
class BWFCodec {
  ...
  static generateBWF(audioBuffer, metadata = {}, options = {}) {
    const sampleRate = options.sampleRate || audioBuffer.sampleRate;
    const numberOfChannels = audioBuffer.numberOfChannels;
    const bitDepth = options.bitDepth || 16; // Use 16-bit for testing to reduce memory
    const numberOfSamples = audioBuffer.length;
    ...
    // BWF structure calculation with correct bext size
    // bext data fields: 256+32+32+10+8+8+2+64+10+180+256 = 858 bytes
    const bextDataSize = 858;
    const riffHeaderSize = 12; // 'RIFF' + size + 'WAVE'
    const bextChunkHeaderSize = 8; // 'bext' + size
    const fmtChunkSize = 24; // fmt chunk header (8) + fmt data (16)
    const dataChunkHeaderSize = 8; // 'data' + size
    ...
    view.setUint32(offset, bextDataSize, true); offset += 4;
  ...
  static parseAudioFile(buffer, filename) {
    try {
      ...
      switch (format) {
        ...
        case 'WAV':
          return { format: 'WAV', detected: true };
        default:
          return { format: 'UNKNOWN', detected: false, error: 'Unsupported file format' };
      }
    } catch (error) {
      return { format: 'UNKNOWN', error: error.message, detected: false };
    }
  }
