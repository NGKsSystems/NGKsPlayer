...
class AdvancedAudioFXEngine {
  ...
  initializeFXChains() {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      // Set up dry/wet mixing with equal-power crossfade starting at full dry
      chain.dryGain.gain.value = 1.0;  // Full dry signal
      chain.wetGain.gain.value = 0.0;  // No wet signal
      
      // Connect dry path: chain input -> dryGain -> chain output
  ...
  destroy() {
    ...
    try { this.output.disconnect(); } catch (e) { console.debug('AdvancedAudioFXEngine: output.disconnect skipped:', e?.message || e); }
    
    // Clean up FX chains
    this.fxChains.forEach(chain => {
      try { chain.input.disconnect(); } catch (e) { console.debug(`AdvancedAudioFXEngine: chain ${chain.id} input.disconnect skipped:`, e?.message || e); }
      try { chain.output.disconnect(); } catch (e) { console.debug(`AdvancedAudioFXEngine: chain ${chain.id} output.disconnect skipped:`, e?.message || e); }
      chain.effects.forEach(fx => {
        try { if (fx.effect.input && fx.effect.input.disconnect) fx.effect.input.disconnect(); } catch (e) { console.debug('AdvancedAudioFXEngine: fx.effect.input.disconnect skipped:', e?.message || e); }
        try { if (fx.effect.output && fx.effect.output.disconnect) fx.effect.output.disconnect(); } catch (e) { console.debug('AdvancedAudioFXEngine: fx.effect.output.disconnect skipped:', e?.message || e); }
      });
    });
